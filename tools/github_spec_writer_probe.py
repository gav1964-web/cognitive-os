"""Run Project Analyzer -> ArchitectSkill -> SpecWriterSkill over cloned GitHub projects."""
...
def main() -> int:
    ...
    parser.add_argument("--label", default="github_spec_writer_probe")
...
def _run_case(project_dir: Path) -> dict[str, Any]:
    ...
    evidence = [str(row.get("source")) for row in spec.get("source_evidence", []) if isinstance(row, dict)]
    ...
    return {
        ...
        "acceptance_count": len(spec.get("acceptance_criteria", [])),
...
def _quality_score(
    ...
    if candidate:
        score += 0.2
    if ranked and ranked[0] == candidate:
        score += 0.2
    if candidate in evidence and evidence:
        score += 0.2
    if not forbidden:
        score += 0.2
