"""Run a read-only materializer audit for exception-pickle active reuse."""
...
from runtime.exception_pickle_materializer_audit import (  # noqa: E402
    DEFAULT_MATERIALIZER_AUDIT,
    run_exception_pickle_materializer_audit,
)
...
def main() -> int:
    ...
    parser.add_argument("--output", default=str(DEFAULT_MATERIALIZER_AUDIT))
    ...
    if args.write:
        ...
        if output == DEFAULT_MATERIALIZER_AUDIT:
            stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S%fZ")
            output = output.with_name(f"exception_pickle_materializer_audit_{stamp}.json")
...
def _summary(report: dict) -> dict:
    return {
        ...
        "unsupported_case_count": report.get("unsupported_case_count"),
        "unsupported_parameter_count": report.get("unsupported_parameter_count"),
        "recommendation_summary": report.get("recommendation_summary"),
        "top_recommendations": list(report.get("recommendations") or [])[:12],
