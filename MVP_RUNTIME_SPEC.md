...
Plugin является каталогом, а не одиночным файлом. Каталог содержит код, схемы, локальный manifest и контрактные тесты. Это позволяет не раздувать entrypoint, держать схемы отдельно от исполнения и декомпозировать код, если реализация приближается к лимиту 400 строк. Repo-level source lint представлен `runtime/repo_lint.py` и `tools/check_repo_lint.py`: каждый Python source file в runtime/tools/plugins/tests и checked-in generated sample projects обязан оставаться <= 400 строк; embedded project templates выносят большие payload в `runtime/template_assets/`, а не хранят их в Python-коде.
...
Contract Registry реализуется в `runtime/contract_registry.py` как производный read-only catalog поверх plugin manifests, JSON Schemas, межслойных packet routes и role artifact APIs. Pipeline validation обязана проверять capability не только по наличию в Capability Registry, но и по executable contract: lifecycle `active|degraded`, object input/output schemas, `additionalProperties=false` и допустимый side-effect manifest. Packet contracts используют тот же enforcement-подход: route/payload валидируются до сохранения в goal report. Role artifacts публикуют producer, consumers и required fields; это API между ролями, а не каталог произвольных отчетов. Для анализа смешанного каталога `ProjectMapReport` может породить `ScopeSelectionReport`; в этом случае `ArchitectureDecisionRecord` и `TechnicalSpec` не строятся до явного выбора active root. Если scope-кандидат имеет сильный score gap или совпадает с очевидным named-package root, foundation pipeline может сам сформировать `ActiveRootDecision` с `source=auto_safe_scope_selector`. Выбор active root фиксируется как `ActiveRootDecision`, после чего foundation pipeline запускает роли только на выбранном подкаталоге и сохраняет исходный portfolio root как audit context. SyntaxError от parser drift внутри уже выбранного active root является health risk и evidence для отчета, но не самостоятельной причиной повторного scope block без inaccessible paths или packaged-copy сигналов.
...
Project fact questions исполняются через декларативный `ProjectFactQuerySpec`. Файл `config/project_fact_query_patterns.json` описывает распознаваемые question patterns, filters, sort, limit и output fields; capability `project_fact_questions` только применяет этот контракт к уже собранным facts. Symbol queries по AST-структуре исполняются через `config/project_symbol_query_patterns.json`: функции, классы и методы становятся queryable symbols с фильтрами по имени/пути/строкам и группировкой по файлам. Source snippet для функции допускается только как bounded read по точному AST symbol (`path`, `name`, `line/end_line`), а не как произвольное чтение файла. Call-level explanation допускается только внутри такого bounded snippet: runtime считает occurrences конкретного call target, возвращает строки и применяет declarative hints для краткого смысла вызова. Текстовые project facts вроде портов, endpoint hints или конфигурационных значений извлекаются через `config/project_text_fact_extractors.json`, где задаются file filters, regexes, scoring и shape ответа. Import/effect project facts вроде использования библиотек или работы с диском извлекаются через `config/project_import_fact_extractors.json`. Source-code signal facts вроде auto-selection, cache wiring или derived recommendations извлекаются через `config/project_code_fact_analyzers.json`, где задаются candidate paths, line signals, cases и derived answers. Новые варианты вопросов должны добавлять или уточнять pattern/extractor/analyzer/KB/role records, а не создавать новую Python-ветку под конкретный prompt. Если pattern отсутствует, дальнейший путь: semantic gap -> L4.5 hypothesis -> проверяемый config proposal или developer improvement request.
...
SpecWriter target quality is checked separately by `runtime/target_quality.py`. `TechnicalSpec.extraction_contract.semantic_quality` records `strong`, `acceptable`, `suspicious`, `poor` or `blocked` for the selected candidate. This is an advisory semantic/ranking signal over evidence, not an execution permission: it rewards bounded parser/factory/resolver/normalizer/query contracts and flags meta-infrastructure, runtime/API boundaries, trivial accessors and support helpers for review. `tools/github_spec_writer_probe.py` reports this signal and treats structurally valid but semantically weak selected targets as `needs_review`.

The foundation-role target is no longer "MVP-ready". The hardening track is `Project Analyzer -> Architect -> SpecWriter >= 9.5/10` before downstream role expansion. `tools/role_foundation_excellence.py` is the strict gate for this track. It scores Project Analyzer on fact recall/precision and GitHub read-only coverage, Architect on judgment quality plus evidence precision/recall, and SpecWriter on curriculum score, GitHub quality, semantic target quality and hygiene. This gate is intentionally stricter than `tools/role_mvp_readiness.py`; passing MVP readiness does not mean the first three roles are accepted as 9.5-level. For GitHub Project Analyzer probes, runtime entrypoints and package/API library surfaces are separate evidence classes: `entrypoints_present` records explicit CLI/service/script boundaries, while `library_surface_present` is allowed only for Python package projects with packaging evidence and a non-empty capability model. The combined coverage field is `entrypoint_or_library_surface_present`. GitHub quality also includes `avg_project_report_quality`, which penalizes generic task statements, thin scenarios, weak execution paths, thin capability models and missing data lifecycle. SpecWriter strict semantic scoring discounts `acceptable` targets heavily; a structurally valid target is not treated as 9.5-level unless the first slice is genuinely strong. TechnicalSpec generation applies bounded semantic rerank: if the structural top candidate is not strong and a nearby ranked candidate is clearly stronger, SpecWriter may select the stronger bounded contract and records the rerank reason. ProjectMapReport minimal extraction suppresses whole-workflow wrappers such as `run_pipeline`/`scrape` when at least three narrower candidates remain; those wrappers still appear as orchestration evidence, not first extraction targets.

Greenfield role planning is represented by `runtime/greenfield_role_pipeline.py` and `tools/greenfield_role_run.py`. It handles new-product prompts without pretending that a source project already exists: `UserPrompt -> ProductArchitectureRecord -> ProductTechnicalSpec`. The output includes human-readable architecture and ТЗ documents under `artifacts/roles/greenfield/`. `ProductArchitectureRecord.open_question_policy` controls Architect launch behavior. The default CLI mode is `--question-mode continue_with_assumptions`: Architect records open questions but still hands off to SpecWriter. `--question-mode ask_user` switches to `ask_user_before_spec`: Architect writes `ProductArchitectureRecord`, report status becomes `needs_clarification`, `next_action=ask_user_clarification`, `clarification_prompt` is returned, and `ProductTechnicalSpec` is not generated until the user supplies clarification. `tools/verified_system_package.py --planning-only` exposes the same greenfield Architect/SpecWriter contour through the normal Stage 2 user prompt CLI without invoking Implementer, Programmer Executor, Foundry or package build. Architecture patterns are loaded from `config/greenfield_architecture_patterns.json`; code is an interpreter of pattern records, not a hardcoded role/domain table. Pattern records now include `research_hints` and `architecture_options`: hints name facts/sources that must be checked before implementation, while options carry chosen/rejected decisions and tradeoffs into SpecWriter. `runtime/config_doctor.py` loads this catalog as part of external config integrity. The first supported domain-specific probe is an OpenVPN client-management server prompt; the artifact must identify API/CLI boundary, profile/config builders, certificate authority adapter, OpenVPN process adapter, audit store, OpenVPN Management Interface / Access Server / PKI research hints, rejected protocol-from-scratch option, secret-handling policy, idempotency/replay policy and acceptance criteria. Implementation remains out of scope for this gate.

Greenfield prompt benchmark v0.1 is represented by `benchmarks/greenfield_prompts/cases.json`, `runtime/greenfield_prompt_benchmark.py` and `tools/greenfield_prompt_benchmark.py`. It scores Architect/SpecWriter planning across new-project prompts only, currently covering 20 OpenVPN service, strict clarification, FastAPI CSV aggregation, file conversion, OCR/image, JSON diff and image-table-to-Excel planning prompts. Acceptance requires every case to match expected status, selected `pattern_id`, required components, primary contract, optional research/option markers, open-question policy and safety invariants (`implementation_started=false`, `source_project_modified=false`, `registry_changed=false`). Each case also includes `GreenfieldArtifactQualityReport`, which must pass checks for specific/non-generic architecture, component contracts, interfaces, data model/lifecycle, research hints, chosen/rejected architecture options, mitigated risks, open questions, TechnicalSpec requirements, chosen architecture option, error model, acceptance criteria, verification strategy and bounded handoff. The same gate renders the human Markdown documents and requires architecture sections `Архитектурное решение`, `Research Hints`, `Архитектурные варианты`, `Первый полезный срез`, `Допущения до уточнения`, `План проверки`, `Открытые вопросы`; and TechnicalSpec sections `Решение для реализации`, `Выбранный архитектурный вариант`, `Исследовательские подсказки`, `Главный контракт`, `Данные и жизненный цикл`, `Стратегия проверки`, `Открытые вопросы для ревью`. It also rejects untranslated default intake questions in Russian-facing documents. The benchmark must not write implementation code or treat its teacher expectations as product ground truth.
...
Config diagnostics представлены отдельными maintenance commands:

```text
python tools/config_doctor.py --root .
python tools/config_coverage.py --root .
python tools/config_mutation_sandbox.py --root . --proposal proposal.json
```

`config_doctor.py` возвращает `ConfigDoctorReport` и проверяет загрузку внешних каталогов, ссылки между role directory, Stage 2 routes, L4/L4.5 rules, OperationRecipe, interface contracts, sandbox operations и sandbox attempt policy. `config_coverage.py` возвращает advisory `ConfigCoverageReport`: отсутствие покрытия не блокирует runtime, но показывает, какие templates/rules/profiles пока не доказаны tests/trial corpus. `config_mutation_sandbox.py` возвращает `ConfigMutationSandboxReport` и валидирует proposed `replace_file` JSON без изменения активного target. `VerifiedSystemPackage` дополнительно содержит `RuleTrace`, который фиксирует config sources и решения, участвовавшие в маршрутизации.
...
Человекочитаемая интерпретация относится к Уровню 4: `runtime/project_deliberation.py` получает expanded bounded deterministic facts плюс `level35_project_signals` и возвращает `level4_project_interpretation` с ключами `executive_summary`, `capability_decomposition`, `refactor_plan`, `cognitive_loop`, `open_questions`, `confidence`. Поверх этого `runtime/project_tasks.py` детерминированно синтезирует `analysis_tasks`: proposed backlog из сигналов L3.5 и L4 refactor/open-question hints. Каждая задача имеет `task_id`, `type`, `title`, `target`, `priority`, `status=proposed`, `source`, `evidence`, `acceptance`; она не исполняется автоматически и не меняет runtime state. После backlog `runtime/project_architecture_synthesis.py` сохраняет `architecture_synthesis`: `ProjectArchitectureSynthesis` с проектным профилем, диагнозом, целевой архитектурной формой, top bottlenecks, matched capability patterns, matched risk patterns, relevant lessons, recommended first slice, deferred scope, verification plan и evidence. Этот слой работает как knowledge-backed rule engine: он загружает `knowledge/architecture_patterns/project_archetypes.json`, `capability_patterns.json`, `risk_patterns.json` и `project_lessons.json`, выбирает декларативное правило/паттерны по фактам, сохраняет `matched_rule`/`matched_because` и интерпретирует recipe через ограниченный runtime-код. `source_scope_policy` в этой базе знаний понижает docs/examples/tests/extras/docs_src как context-only evidence, чтобы first-slice targets выбирались из core source при наличии альтернатив. Knowledge records могут расти независимо от Python-кода, но не содержат произвольного исполняемого кода. Уровень 4 может использовать только отдельную cortex model с большим контекстным окном через `--l4-model`, `--l4-base-url`, `--l4-api-key-env` и `--l4-context expanded|compact`; default-профиль L4 использует `deepseek/deepseek-chat` через OpenAI-compatible gateway `http://127.0.0.1:8000/v1`, а `COGNITIVE_OS_L4_MODEL` и `COGNITIVE_OS_L4_BASE_URL` позволяют его явно переопределить. Локальные модели Уровня 3.5 (`local`, `qwen-local`, GGUF-профили) для L4 запрещены. Если expanded context не помещается в выбранную cortex model, L4 может один раз повторить запрос в compact mode и пометить `context_mode=compact_after_overflow`. Если cortex model недоступна, L4 возвращает controlled `deterministic_fallback` и не выполняет локальный LLM-вызов. Все project-analysis LLM artifacts advisory: они не меняют Pipeline DSL, Capability Registry, execution state и не подменяют фактические секции отчета.
...
python tools/goal_run.py --root . --goal "Analyze project map" --execute --interpret-project-llm --input-json "{\"path\":\"F:\\\\ubuntu\\\\test\\\\map\"}" --l4-model deepseek/deepseek-chat
...
`knowledge_preflight` работает иначе, чем memory preflight. Он запускается только при явном `KnowledgeGap` или при route/role policy, где missing fact блокирует решение. Разрешенные MVP-источники: user clarification, local files already in scope, installed package probe, allowlisted official docs fetch, GitHub repository search as external inspiration/evidence, goal/dialog memory, existing reports. Реализованные sources: `inspect_installed_packages` для локальных backend facts, `official_docs_fetch` для official documentation excerpts с source policy и content hash, `github_repository_search` для поиска сопоставимых проектов без копирования кода.

Для project-analysis используется `runtime/research_loop.py`: generic/weak architecture synthesis (`python_project` или low confidence) создает `KnowledgeGapPacket` и `ResearchPlan`. По умолчанию план не исполняет network lookup автоматически; он описывает bounded acquisition steps и возвращает `SourceDigest` только после явного запуска разрешенной capability. Внешние источники являются evidence, не ground truth.

KB пополняется через admission gate (`runtime/knowledge_admission.py`). Новый record сначала становится staged `KnowledgeCandidate`; active KB merge требует нескольких подтвержденных кейсов, подтверждения внешнего учителя/корректировщика и Codex/developer approval. `runtime/knowledge_review_console.py` и `tools/knowledge_candidates.py review` строят очереди review: `collect_more_cases`, `needs_teacher_approval`, `needs_codex_approval`, `ready_for_human_merge`. Автоматическое само-продвижение candidate в KB запрещено.

Staged candidates сохраняются в `artifacts/knowledge_candidates/*.json` и управляются через `tools/knowledge_candidates.py`. Эти файлы являются review queue, а не активной KB: runtime не загружает их как rules/patterns до явного human merge в `knowledge/architecture_patterns/*.json`.
...
52. Role Skills v0.7 разделяют профессиональные когнитивные роли от runtime capabilities. Роли не являются source-code constants. Активный source of truth - `config/role_directory.json`: каждая запись роли содержит identity, label, description, capabilities, consumes/produces, KB filters, policy, optional artifact_builder, а общий раздел `pipeline` задает chain settings. В `role_directory.v2` каждая роль также обязана объявлять `contract`, `gates`, `fallback_policy`, `llm_policy`, `kb_policy`, `stop_conditions` и `quality_criteria`; `runtime/role_operational_policy.py` проверяет полноту этих операционных полей, а `runtime/role_gate_runner.py` исполняет configured gates/quality criteria против produced artifacts и добавляет `RoleGateReport` в role pipeline output. Python-код является интерпретатором этого справочника: `runtime/role_directory.py` валидирует directory, `runtime/role_artifact_interpreter.py` исполняет directory pipeline, `runtime/role_artifact_builder.py` вызывает artifact_builder из записи роли, `runtime/configured_role_pipeline.py` запускает configured pipeline prefixes по artifact type, `runtime/role_definitions.py` строит role definitions/default scopes из directory, а `runtime/role_skills.py` экспортирует только generic `run_role_skill(role_id, **inputs)`. Role-specific Python facades (`run_architect_skill`, `run_spec_writer_skill` и т.п.) запрещены как active API; legacy split files `roles/*.json`, `config/artifact_builders.json`, `config/role_artifact_pipeline.json`, `config/role_record_defaults.json` могут оставаться только как migration/import material до удаления и не являются authority. `registry/skills.json` описывает L4 role skills как metadata и указывает generic entrypoint плюс role arg. CLI `tools/role_skill_run.py` не содержит ручной цепочки ролей: он выбирает runnable step и bindings из `config/role_directory.json`, подставляет входные artifacts и вызывает generic role interpreter. Role skill не имеет права писать код, менять Capability Registry, исполнять pipeline или promote candidates. MVP acceptance проверяет deterministic role-skill path без обязательного LLM-вызова; LLM может быть добавлен позже как advisory backend роли, но typed artifact contract остается обязательным. ImplementerSkill в этой цепочке означает только Implementer Planner. Его `ImplementationPlan` обязан включать `ImplementationBlueprint`, `PatchIntent` и `ExecutorHandoff`: это API-поля для programmer executor, а не пояснительный текст. Роль `programmer_executor` представлена `runtime/programmer_executor.py` и `tools/apply_implementation_plan.py`: она берет `ImplementationPlan`, создает isolated `PatchPackage`, переносит `ImplementationBlueprint/PatchIntent/ExecutorHandoff` в package, сохраняет `source_snapshot`, запускает allowlisted verification commands, пишет `TestResult` и может быть включена в role pipeline через `--run-executor`, после чего Reviewer получает `test_result`. `runtime/programmer_patch_synthesizer.py` добавляет первый bounded coding pattern: по Tester obligations для простого `file.py:function` target он может создать sandbox-копию проекта, предложить `PatchOperation` с unified diff для required-input guard и прогнать executable acceptance на patched sandbox. Executor всегда добавляет project-scoped verification по `expected_files`: Python-файлы проверяются через `py_compile` внутри execution project/sandbox, что дает evidence по конкретному изменяемому target. Общий executor не применяет изменения к source: `--apply-source` возвращает controlled block. Отдельный узкий контур `tools/sandbox_patch_review.py --apply-approved` может применить только прошедший проверку специализированный sandbox package после явного одобрения, проверки source identity/invariants и создания timestamped backup; это не универсальный Programmer Executor и не разрешение на автономное редактирование произвольного проекта.

52.0.1. Programmer Field Trial представлен `runtime/programmer_field_trial.py` и `tools/programmer_field_trial.py`. Он прогоняет реальные проекты через `Role Pipeline + Programmer Executor` и оценивает именно программиста: `planning_score`, `execution_score`, `coding_score`, `safety_score`, `maturity_score`. Verdicts: `planner_only`, `executor_only`, `programmer_active`. На текущем MVP роль программиста классифицируется как `programmer_active` только для поддержанных узких patch patterns: она умеет принимать bound ImplementationPlan, создавать PatchPackage/source snapshot, предлагать sandbox-only PatchOperation и запускать verification. Для остальных классов задач она должна честно возвращать `patch_synthesis.status=skipped|blocked`, не применяя source edits. Field trial также проверяет `verification_project_scoped`; passing root-level smoke вроде `mvp_acceptance` не считается доказательством, что проверен целевой проект.
...
52.0.3. Greenfield Scaffold + Code Generation v0.1 представлен `runtime/greenfield_scaffold.py`, базовым `runtime/greenfield_templates.py`, расширенным `runtime/greenfield_local10_templates.py` и Stage 2 шаблонами `runtime/greenfield_stage2_templates.py`; он включается в Programmer Prompt Curriculum при `write=True`. Scaffold отвечает за sandbox root, path safety, запись файлов и verification; templates отвечают за содержимое generated modules/tests. Для стартовых трех curriculum cases генератор пишет минимальную deterministic реализацию: ixbt fixture parser + CSV writer, Markdown->RTF converter + CLI, XLSX<->CSV adapter на стандартной библиотеке (`zipfile` + XML) с явной legacy `.xls` policy `unsupported_without_adapter`. Для Local-10 он генерирует небольшие CLI/package проекты с чистыми core transforms, fixtures, CLI entrypoint и project-scoped tests. Для Stage 2 он также умеет генерировать FastAPI CSV aggregator package с `app.py`, core aggregator, API tests, fixture CSV и README/run command. Для простых file-conversion prompts вида `.source -> .target` используется общий `GenericFileConversionRecipe`: из prompt извлекаются source/target extensions, строится синтетический reference, один reusable `file_converter_cli` scaffold, adapter boundary и dependency-free fixture tests; отдельная KB/template-запись на каждую пару форматов не создается. Рядом формируется `LibraryBindingRecipe`: advisory artifact с кандидатами backend/library pipeline, selection policy, verification policy и authority flags (`may_install_dependencies=false`, `may_call_network=false`), то есть список библиотек является гипотезой для будущего adapter work, а не действием. `AdapterImplementationPlan` выбирает реализуемый backend: безопасные stdlib adapters (`stdlib_text_to_html`, `stdlib_markdown_subset_to_rtf`, `stdlib_image_to_doc_html`) генерируются в `src/file_converter_cli/adapters.py` и покрываются semantic adapter tests; тяжелые пары остаются `fallback_only` через fixture adapter до явного dependency decision. После записи запускаются project-scoped `python -m compileall -b .` и `python -m pytest tests -q`; `code_file_generation`, `dependency_policy` и `project_scoped_tests` считаются покрытыми только если verification прошла. Все пути артефактов валидируются и не могут выйти за пределы scaffold root; source tree и registry не меняются.
...
52.0.5. Stage 2: Verified System Package является следующим целевым рубежом поверх MVP, а не заменой текущего runtime. Он представлен `runtime/prompt_adequacy.py`, `tools/prompt_adequacy.py`, `runtime/verified_system_package.py` и `tools/verified_system_package.py`. Входом является bounded adequate prompt; первым шагом всегда выполняется `PromptAdequacyGate`, который классифицирует prompt как `ready`, `needs_clarification`, `unsupported` или `too_broad` по проверяемым полям: понятная цель, определенный тип системы, входы/выходы, ограничения, dependency policy, проверяемые success criteria и bounded scope. `PromptAdequacyGate` не является текстовым отчетом: это API-вход для `L4.0 CognitiveControlPlaneDecision` в режиме `prompt_to_product`. Stage 2 может продолжить сборку только если `prompt_product_gate.status=passed` и `role_transition.next_action=build_verified_system_package`; иначе возвращается clarification/blocker или controlled stop вместо генерации. Если prompt `ready`, но нет поддержанного package template, L4.0 фиксирует `no_supported_package_template` как bounded semantic escalation candidate для L4.5, а не разрешение обходить contracts. Если prompt `needs_clarification`, но выглядит как concrete bounded implementation request без risk/unsupported markers, L4.0 фиксирует `prompt_intake_uncertainty` и тоже вызывает L4.5. Если prompt является bounded behavior/limitation question по поддержанному домену, L4.0 фиксирует `behavior_question_uncertainty` и тоже вызывает L4.5; L4.5 должен ответить только из evidence или вернуть `DeveloperImprovementRequest` на fact-based behavior-question answering. Vague prompts и prompts с secrets/live-network risks остаются обычным `ask_clarification`. L4.5 в этих ветках сначала пробует existing means по `SemanticEvidencePack`: если найден существующий проверяемый маршрут, сохраняется `SuccessfulResolutionCandidate` с verification plan и слабым KB candidate; если маршрут не найден, сохраняется `DeveloperImprovementRequest` для Codex/human work. Ни один из этих artifacts не меняет template catalog или KB автоматически. `tools/verified_system_package.py --use-l45-llm` включает реальный OpenAI-compatible L4.5 proposal через `COGNITIVE_OS_L45_BASE_URL`/`COGNITIVE_OS_L45_MODEL`; default model id - `deepseek/deepseek-chat`, а `response_format` для L4.5 default выключен для совместимости с текущим gateway. Provider failure фиксируется в proposal hardening и падает обратно на deterministic proposal. Если gate `ready` и template поддержан, Stage 2 строит isolated working package: source files, tests, README/run instructions, verification report, known limitations, tester review и release decision. Поддержанные классы в текущем MVP: `CLI / file-processing utility` для JSONL log filter, text stats CLI, CSV sort CLI, OCR image CLI, image contents CLI, image table to Excel CLI и generic file converter CLI, а также `FastAPI / small local service` для CSV aggregation service и in-memory key/value CRUD service с dependency policy `fastapi`. Generic file converter CLI строится из `GenericFileConversionRecipe`, а не из отдельного curriculum case под каждую пару расширений; для `.xls -> .png`, `.md -> .rtf`, `.txt -> .html` и похожих bounded prompts scaffold содержит `conversion_recipe.json`, `library_binding_recipe.json`, `adapter_implementation_plan.json`, reusable `file_converter_cli`, adapter boundary, missing/unsupported error tests и project-scoped `python -m file_converter_cli.cli` smoke. `LibraryBindingRecipe` предлагает bounded adapter candidates (`LibreOffice/headless`, `xlrd` + `Pillow`, `pandoc`, stdlib subset adapters или `custom_adapter_required`) с verification policy и authority flags; это API-артефакт для programmer/L4.5, а не разрешение автоматически ставить библиотеки. `AdapterImplementationPlan` делает следующий шаг: если выбранный backend безопасно реализуется стандартной библиотекой, generated package пишет настоящий adapter code и semantic adapter tests; если нет, план остается `fallback_only` и не имитирует production fidelity. Image contents CLI использует optional OpenAI-compatible vision backend только для real image understanding; image table to Excel/CSV/HTML/DOC/RTF CLI использует injectable OCR/text backend, `--ocr-text-file` для воспроизводимого real-image workflow, optional OpenAI-compatible vision/OCR backend, stdlib OOXML writer, CSV writer, legacy XLS-compatible HTML writer, plain HTML writer, DOC-compatible HTML writer и RTF writer, а default tests обязаны работать без сети и без OCR/spreadsheet dependencies. FastAPI packages обязаны иметь health/domain endpoints, JSON responses, controlled HTTP errors и project-scoped API/core tests. Stage 2 corpus живет в `curricula/programmer_prompt_stage2/*/teacher_reference.json`, а generic converter uses a synthetic teacher reference from the recipe; acceptance включает `verified_system_package_cli`, `verified_system_package_fastapi`, `verified_system_package_text_stats`, `verified_system_package_fastapi_kv`, `stage2_template_admission_csv_sort`, `verified_system_package_csv_sort`, `verified_system_package_ocr_image_cli`, `verified_system_package_image_contents_cli`, image-table-to-Excel package runs и generic file-converter package runs. Если tester запрашивает rework, запускается только bounded contract debug loop `runtime/stage2_debug_loop.py`: `FailureAnalysis -> ReworkPlan -> sandbox repair -> verification -> tester review`. Loop применяет только allowlisted deterministic repairs внутри isolated generated package: восстановление README prompt/run-command alignment, dependency policy в `pyproject.toml` для FastAPI package, CLI entrypoint contract (`argparse`, positional `input/output`, вызов `run_cli`), negative/edge test evidence (`empty` для text stats, malformed `not-json` для JSONL filter) и controlled HTTP error (`400` для CSV validation, `404` для key/value missing item); неизвестные сбои остаются `blocked`. `tools/stage2_debug_loop_probe.py` намеренно повреждает generated package, проверяет, что tester фиксирует failure, затем требует successful bounded repair и повторную project-scoped verification; MVP acceptance включает FastAPI, CLI и edge-case probes как отдельные L4 programmer gates. Direct modification of the user's source project remains forbidden until explicit human approval: `VerifiedSystemPackage.invariants.direct_user_source_modification=false`, `human_approval_required_for_source_apply=true`. Каждый successful `VerifiedSystemPackage` содержит `ProgrammerSandboxGate`, который проверяет generated project dir, passed verification, tester approval, `source_tree_unchanged=true` и `registry_unchanged=true`; release-ready без этого gate считается недействительным.

52.0.5.1. Stage 2 Fallback Autonomy Loop v0.1 представлен `runtime/fallback_autonomy_loop.py` и закрывает правило: deterministic KB/schema route -> L4.5 hypothesis -> `SandboxAttemptSpec` -> bounded sandbox attempt -> verified package или developer handoff. Loop запускается только после `SemanticHypothesisProposal` и `L4SemanticValidationResult` с accepted action `record_successful_resolution_candidate`. Он не исполняет произвольный model output и не генерирует новый template: proposal должен мапиться на уже существующий Stage 2 route. `runtime/sandbox_attempt_spec.py` строит typed API artifact `SandboxAttemptSpec` с allowlisted `attempt.kind`, `case_name`, runner `ProgrammerProjectReview`, project-scoped verification commands, allowed operations, forbidden operations и invariants. Policy вынесена в `registry/sandbox_attempt_policy.json`: код является интерпретатором policy, а не местом хранения списка sandbox routes. Если registry policy отсутствует, невалидна или не active, runtime использует минимальную safe default policy и не выдает дополнительных прав. Поддержанные kind v0.1 в active policy: `existing_stage2_case` для `image_contents_cli` / `csv_sort_cli` и `bounded_adapter_recipe` для `generic_file_converter_cli`. `bounded_adapter_recipe` вкладывает `GenericFileConversionRecipe`, `LibraryBindingRecipe` и `AdapterImplementationPlan`; validation разрешает только fixture или `stdlib_*` backend, требует authority flags `may_install_dependencies=false`, `may_call_network=false`, `may_edit_user_source=false` и блокирует dependency install, network, registry/source/KB mutations, arbitrary shell и model-generated code execution до запуска sandbox. Для generic converter reference строится синтетически из recipe, потому отдельная curriculum папка на каждую пару форматов не нужна. Validation блокирует non-allowlisted cases, отсутствующий curriculum reference для обычных existing cases, произвольный runner, произвольные shell commands, dependency install/network/model-code execution и любые source/registry/KB mutations до запуска sandbox. Если model-backed L4.5 возвращает валидную, но слабую гипотезу, а deterministic existing-means resolver по evidence находит проверяемый маршрут, hardening обязан заменить proposal на `successful_existing_resolution` и зафиксировать `deterministic_existing_route_rescue=true` вместе с исходным `model_hypothesis_type`; это не повышает доверие к модели, а не дает модели ухудшить доказуемый маршрут. После успешной validation runner вызывает обычный `ProgrammerProjectReview` в isolated scaffold, затем использует прежнюю tester verification. Если verification проходит, `VerifiedSystemPackage` получает `fallback_autonomy_loop.status=sandbox_verified`, `release_decision=release_ready`, вложенный `sandbox_attempt_spec`, `SuccessfulResolutionCandidate` и staged `KnowledgeCandidate` со статусом `collect_more_cases`; при `write=true` этот кандидат сохраняется в `artifacts/knowledge_candidates`. Если mapping/spec validation отсутствует или verification падает, результат остается `developer_handoff_required` / `DeveloperImprovementRequest`. Инварианты loop: `sandbox_only=true`, `llm_may_propose_not_execute=true`, `source_tree_changes=false`, `registry_changes=false`, `kb_promotion=false`. KB admission по-прежнему требует нескольких подтвержденных кейсов и review старшим разработчиком; один успешный прогон является кандидатом, а не знанием.

52.0.5.1a. Profile-gated news-site scraper CLI v0.6 представлен route `news_site_scraper_cli`, teacher reference в `curricula/programmer_prompt_stage2/news_site_scraper_cli`, profile catalog `config/web_extraction_profiles.json`, routing interpreter `runtime/web_extraction_profiles.py` и template `runtime/greenfield_news_scraper_template.py`. Он заменяет подход "один scraper template на один сайт" для задач вида `https://3dnews.ru/ -> CSV`, но не считает любой URL надежно поддержанным новостным сайтом. Prompt классифицируется как `cli`; package route может использовать known-host profile или generic `SiteProfile`. Сам `SiteProfile` является контрактом извлечения: `base_url/source`, `link_markers`, `article_path_patterns`, `blocked_path_markers`, `blocked_titles`, same-host policy и фильтры служебной навигации. Для неизвестных hosts generic profile разрешает только article-like candidates и обязан вернуть controlled failure, если live page не дает запрошенный top count; это лучше, чем выдавать служебные ссылки как новости. Generated package содержит allowlisted `STRATEGY_REGISTRY`: `html_links`, `json_ld`, `rss_atom`, `sitemap`, `browser_dom` и diagnostic-only `api_probe`; L4.5 может рекомендовать strategy type, но runtime исполняет только эти bounded стратегии. Parser работает fixture-first по `article`, schema.org JSON-LD (`NewsArticle`/`Article`/`BlogPosting`), embedded JSON, RSS/Atom feed discovery, sitemap discovery и fallback news links; CSV/Markdown/JSON contract фиксирован (`title,url,date,summary,source`), live network остается optional smoke через fetcher adapter с timeout/user-agent. RSS/Atom fallback использует declared `<link rel="alternate">` feeds и bounded common feed paths вроде `/rss.xml`, `/feed.xml`, `/ru/rss/articles/`; sitemap fallback использует same-host `/sitemap.xml`, `/sitemap-news.xml` и найденные sitemap XML URLs, но пропускает только article-like URLs через тот же profile contract. Если HTML/JSON-LD/RSS/sitemap paths не набрали requested top count, generated CLI включает bounded LLM fallback по OpenAI-compatible endpoint (`COGNITIVE_OS_NEWS_LLM_BASE_URL` или `COGNITIVE_OS_L45_BASE_URL`, default `http://127.0.0.1:8000/v1`; model default `deepseek/deepseek-chat`). LLM получает только fetched evidence: page text, same-host links и requested top; output contract `{items:[{title,url,date,summary}]}`. Результат модели не исполняется и не принимается на веру: deterministic validator отбрасывает внешние URLs, служебные заголовки, time/action cards и дубли, затем прежний top-count gate решает success/failure. CLI пишет stderr trace `attempting LLM fallback`, чтобы факт обращения был виден в логах. `--no-llm-fallback` отключает сетевую модель для тестов и offline режима. Default verification не требует сети и проходит project-scoped compile/pytest; release decision может быть `release_ready_with_risks` из-за сетевого live-smoke риска.

52.0.5.1b. News LLM fallback должен работать как семантический диагност отказа, а не как свободный parser. Generated package нормализует model output в `SiteDiagnosis` со вложенным `StrategyCandidate`; допустимые strategy types: `direct_evidence_items`, `feed_candidate`, `json_candidate`, `browser_required`, `unsupported`. `direct_evidence_items` и `json_candidate` могут дать временные news candidates только через deterministic same-host/no-navigation validation. `feed_candidate` может дать feed URLs, но runtime сначала валидирует same-host allowlist и только потом fetches/parses RSS/Atom через stdlib; feed probing имеет короткий per-feed timeout и общий budget, чтобы 403/anti-bot страницы не подвешивали CLI. `browser_required` не исполняет произвольный model output: CLI имеет явный operator-controlled режим `--browser auto|always|off` и `--browser-timeout-ms`. `auto` пробует browser-render retry только после L4.5 диагноза `browser_required` или deterministic browser-challenge markers, `always` пробует rendered extraction до LLM fallback при недостатке HTML/RSS rows, `off` запрещает browser path. Browser backend объявлен как optional dependency `.[browser]` (`playwright`) и требует отдельного `python -m playwright install chromium`; отсутствие backend является controlled condition, а не runtime bug. После rendered fetch CLI снова применяет обычный parser/feed/same-host validation. Если browser backend отсутствует или validated news не набраны, а также для `unsupported`, CLI пишет `NewsExtractionFailureReport` (`status=blocked`, diagnosis, strategy type, risks, next actions, invariants `no_placeholder_news_written=true`, `model_output_executed=false`) и завершает работу controlled non-zero exit. Таким образом сложный сайт вроде JS/anti-bot protected source становится не ложным успехом, а проверяемым handoff/developer-improvement artifact.

52.0.5.2. Project Change Trial v0.1 обобщает контур "внешний учитель/корректор -> изолированная фикстура -> sandbox patch -> review/apply в фикстуру -> tester verdict" без превращения его в самообучение. Общий каркас живет в `runtime/project_change_trial.py`: он создает fixture из baseline-файлов, переносит optional context files, сравнивает результат с teacher reference по exact diff и feature evidence, пишет отчет и фиксирует invariants `source_project_modified=false`, `fixture_only_apply=true`, `teacher_reference_is_ground_truth=false`, `automatic_code_changes_from_own_output=false`. Конкретные сценарии, например `tools/map_llm_migration_trial.py`, подключают собственный analyzer/patch generator/tester поверх этого каркаса. Teacher reference остается проверяемым ориентиром, а не ground truth: исправление runtime/codebase выполняется отдельным инженерным изменением, а не автоматическим самопатчем по собственному выводу системы.

52.0.5.3. Project Change Scenario Interface v0.1 описывает такие trials декларативным JSON-сценарием и исполняется через `runtime/project_change_scenario.py` / `tools/project_change_trial_run.py`. Поддержанные apply-типы: `copy_teacher_to_fixture` и `sandbox_patch_package`. Для `sandbox_patch_package` builder обязан быть active-записью в `registry/project_change_builders.json`; scenario не может импортировать произвольный модуль или вызвать произвольную shell-команду. Текущий allowlisted builder `gigachat_sandbox_patch` строит sandbox package для fixture, затем проводит `sandbox_patch_review` и применяет результат только в fixture. Source-project immutability проверяется snapshot/hash до и после apply.
...
56. Role Foundation Pipeline v0.2 является ближайшим quality gate для верхнего контура: `ProjectMapReport -> ArchitectureDecisionRecord -> TechnicalSpec`. Он представлен `runtime/role_foundation_pipeline.py` и `tools/role_foundation_run.py`, прогоняется минимум на одном benchmark-проекте и не запускает Implementer/Tester/Reviewer, Foundry, source edits или registry changes. Этот gate считается стабильным, если все три артефакта записаны и проходят deterministic `role_artifact_quality`: ProjectMapReport содержит `source_health` (`clean/noisy/damaged`, `single_project/multi_project_workspace/dirty_portfolio`, syntax/inaccessible/generated signals), source-backed scope, inputs/outputs, entrypoints/runtime commands, execution path, capability candidates, contracts/data, errors/state/reproducibility, data lifecycle и minimal extraction plan; ADR содержит chosen option, tradeoff-ranked architecture options, explained rejected options with score delta/deferred-until condition, first-slice selection policy, subsystem boundaries with inputs/outputs, actionable risks with impact/mitigation/evidence, data lifecycle, state model, traceability, source context и SpecWriter brief with contract targets; TechnicalSpec содержит requirements, interface contracts, source-linked acceptance criteria, traceability table, `source_evidence`, ranked `extraction_contract`, error model, state/replay policy и bounded handoff. `runtime/architect_red_team.py` дополнительно выпускает `ArchitectRedTeamReport`; foundation score падает, если ADR не имеет option tradeoffs, explained rejected options, bounded source-backed first slice, source context для brief targets, source-linked traceability, actionable risks или forbidden-action enforcement. `dirty_portfolio` является sanitation signal: система может написать обзорный документ, но не должна считать весь root обычным extraction target без выбора конкретного проекта. При `write=True` gate обязан также писать `human_documents.architecture_analysis` - Markdown-документ для человека с executive summary, source health, purpose/boundaries, entrypoints, capability candidates, recommended extraction contract, risks, open questions, evidence/traceability, non-goals и next step; и `human_documents.technical_spec` - человекочитаемое ТЗ с scope, выбранным extraction contract, requirements, interface contracts, data lifecycle, error model, acceptance criteria, traceability, state/replay policy, verification strategy и implementation handoff. Эти документы являются reading surface поверх typed JSON artifacts, а не заменой контрактов. При наличии written documents `runtime/human_document_quality.py` обязан выпустить `HumanRoleDocumentQualityReport`; foundation score падает, если документы не сохраняют цепочку `ProjectMapReport -> ArchitectureDecisionRecord -> TechnicalSpec`, русскоязычные секции, evidence/traceability, validation gates, acceptance criteria и implementation handoff. `extraction_contract` обязан выбирать ranked best-first candidate, а не первый валидный symbol: контракт содержит `ranked_candidates`, `candidate_score` и `selection_reason`, а выбранный `candidate` должен совпадать с первой строкой ранжирования. Для benchmark corpus `expected_analysis.json` задает `expected_best_extraction_candidate`; foundation field trial считает `candidate_match_score` и падает, если выбранный candidate не совпал с эталоном. ImplementerSkill подключается к основному пути только после стабилизации этого перехода.
...
57.3. SpecWriter Curriculum External-3 v0.1 переносит тот же gate на реальные локальные проекты `F:/ubuntu/test/map`, `F:/ubuntu/test/5` и `F:/ubuntu/test/004`; corpus живет в `curricula/spec_writer_external_local_3` и запускается через `tools/spec_writer_curriculum.py --curriculum-dir curricula/spec_writer_external_local_3 --write`. Он проверяет, что TechnicalSpec выбирает первый extraction contract по source evidence, а не по поверхностной чистоте функции: global/memory-state helpers, request/middleware handlers, route/API boundaries, packaged copies, tests/tools/scratch и legacy analyzer branches не должны становиться первым candidate. Зафиксированные teacher-oriented ожидания: `map -> import_indoc.py:parse_file`, `5 -> app/providers/factory.py:build_providers_from_config`, `004 -> p0042/service.py:resolve`; соседние provider/LLM parser boundaries должны попадать в ranked/source evidence и acceptance traceability, но не подменять выбранный writable target. External-3 не является portable обязательным fixture, потому зависит от локальных путей разработчика; тест может быть пропущен, если проекты отсутствуют.

57.3.1. TechnicalSpec Quality Gate v0.2: `TechnicalSpec` является API для Implementer, а не prose report. Gate требует source evidence, ranked extraction contract, work-plan contract, interface contracts, source-linked acceptance, traceability, specific I/O shapes и side-effect gates. Слабый `Any -> Any` допускается только как blocked handoff или как domain `contract_family` с typed shapes, validation gates и failure modes. `SpecWriterRedTeamReport` является отдельным deterministic red-team pass: если Implementer должен угадывать входы, выходы, side-effect policy, acceptance для candidate или first-slice traceability, verdict становится `return_to_spec_writer`; только `ready_for_implementer` считается пригодным для downstream handoff.

57.4. Implementer Curriculum Local-3 v0.1 является следующим gate, но не включает реальную реализацию. Он представлен `curricula/implementer_local_3`, `runtime/implementer_curriculum.py` и `tools/implementer_curriculum.py`. Runner строит fresh `ProjectMapReport -> ArchitectureDecisionRecord -> TechnicalSpec -> ImplementationPlan`, после чего scorer оценивает только `ImplementationPlan`: candidate совпадает с `TechnicalSpec.extraction_contract`, `contract_binding` содержит input/output contract, implementation units/change plan/quality gates присутствуют, `ImplementationBlueprint` готов и указывает на candidate, `PatchIntent` имеет `sandbox_first` и `apply_source_default=false`, `ExecutorHandoff` указывает на `tools/apply_implementation_plan.py`, `patch_scope` и `writable_scope` ограничены selected candidate, широкий соседний контекст переносится только в read-only `evidence_scope`, expected files выводятся из writable target, rollback plan покрывает expected files, verification commands и acceptance mapping присутствуют, registry policy запрещает неявные registry edits, forbidden actions пусты, handoff идет к Tester. Foundry, promote, source edits, registry changes, Tester и Reviewer не входят в scope gate. Summary обязан публиковать `avg_score`, `worst_case_score`, `ready_threshold=0.92` и `ready_by_worst_case`; переход дальше оценивается по худшему проекту, а не по среднему score.

57.4.1. Implementer Greenfield Planning v0.1 расширяет только планировщик, а не права executor. Если входной artifact имеет тип `ProductTechnicalSpec`, `runtime/implementation_plan_builder.py` строит `ImplementationPlan` с `implementation_target.mode=greenfield_project`: target `greenfield:<case>`, bounded package layout из Stage 2 route, `greenfield_project_plan`, component-to-file map, implementation units по component contracts, adapter/side-effect policy, fixture-first verification commands, acceptance mapping, conservative dependency policy и sandbox-first `PatchIntent/ExecutorHandoff`. Этот режим связывает `ProductTechnicalSpec -> ImplementationPlan` для prompt-to-product контура, но не генерирует код сам, не меняет user source, registry или KB и не подменяет Tester/Reviewer. `runtime/role_artifact_quality.py` оценивает greenfield target как specific только при bounded writable scope внутри expected files.

57.5. Implementer Curriculum External-3 v0.1 использует те же проверки для реальных локальных проектов `F:/ubuntu/test/map`, `F:/ubuntu/test/5` и `F:/ubuntu/test/004`; corpus живет в `curricula/implementer_external_local_3` и запускается через `tools/implementer_curriculum.py --curriculum-dir curricula/implementer_external_local_3 --write`. Зафиксированные teacher-oriented ожидания: `map -> import_indoc.py:parse_file`, `5 -> app/providers/factory.py:build_providers_from_config`, `004 -> p0042/service.py:resolve`. External gate проверяет, что ImplementationPlan удерживает target из TechnicalSpec, переносит контракты, ограничивает expected files/rollback, не делает source edits, не меняет registry, не вызывает live provider/local LLM и не запускает Foundry/promote. Как и остальные External-3 corpus, он зависит от локальных путей и может быть пропущен в portable test environment.
57.6. Tester Curriculum Local-3/External-3 v0.1 является следующим gate после Implementer Planner и не включает execution. Он представлен `curricula/tester_local_3`, `curricula/tester_external_local_3`, `runtime/tester_curriculum.py` и `tools/tester_curriculum.py`. Runner строит fresh `ProjectMapReport -> ArchitectureDecisionRecord -> TechnicalSpec -> ImplementationPlan -> TestPlan`, после чего scorer оценивает только `TestPlan`: target совпадает с ImplementationPlan target, binding usable, writable scope сохранен, evidence scope вынесен в read-only context, contract matrix присутствует, acceptance/negative/smoke/regression tests присутствуют, executable acceptance obligations готовы, dependency policy соответствует teacher reference, forbidden actions пусты, next role ведет к Reviewer. Filesystem, network, provider, LLM, browser и subprocess boundaries должны быть fake/fixture-by-default для contract tests; live integration допускается только отдельным opt-in smoke. GitHub tester probe применяет тот же Python primary-language gate, что foundation/implementer probes: non-Python-primary workspaces попадают в `out_of_scope`, а не в scored readiness. Source safety измеряется before/after git snapshot, чтобы pre-existing dirty checkout не считался изменением от роли. Summary обязан публиковать `avg_score`, `worst_case_score`, `ready_threshold=0.92` и `ready_by_worst_case`; переход дальше оценивается по худшему проекту, а не по среднему score. External corpus использует `F:/ubuntu/test/map`, `F:/ubuntu/test/5` и `F:/ubuntu/test/004` и зависит от локальных путей разработчика.
57.7. Reviewer Curriculum Local-3/External-3 v0.1 является следующим gate после Tester и не включает execution. Он представлен `curricula/reviewer_local_3`, `curricula/reviewer_external_local_3`, `runtime/reviewer_curriculum.py` и `tools/reviewer_curriculum.py`. Runner строит fresh `ProjectMapReport -> ArchitectureDecisionRecord -> TechnicalSpec -> ImplementationPlan -> TestPlan -> ReviewFindings`, после чего scorer оценивает только `ReviewFindings`: review target совпадает с implementation/test target, binding usable, coverage confirms target, scope preserved, conformance checks passed, contract violations empty, architecture drift empty, risk assessment present, recommendation matches expected policy, forbidden actions empty. Reviewer обязан различать residual risks и blocking rework: medium regression risks дают `approve_with_risks` без rework tasks, а rework появляется только для high findings/risks или failed conformance/TestResult. GitHub reviewer probe применяет Python primary-language gate, считает non-Python-primary workspaces `out_of_scope`, режет quality ниже проходного без usable binding и измеряет source changes по before/after git snapshot. Summary обязан публиковать `avg_score`, `worst_case_score`, `ready_threshold=0.92` и `ready_by_worst_case`; переход дальше оценивается по худшему проекту.
57.8. Full Role Chain Gate v0.1 является сквозной проверкой перед переходом к настоящему Programmer Executor. Он представлен `tools/github_full_chain_probe.py` и прогоняет один configured role pipeline до `ReviewFindings`: `ProjectMapReport -> ArchitectureDecisionRecord -> TechnicalSpec -> ImplementationPlan -> TestPlan -> ReviewFindings`. Gate оценивает не отдельную роль, а связность handoff: наличие typed artifacts, traceability/source context в ADR, source-backed `TechnicalSpec.extraction_contract`, сохранение одного target через Spec/Implementer/Tester/Reviewer, usable binding, bounded writable scope, contract/negative tests, reviewer conformance, отсутствие contract violations, architecture drift, forbidden source targets, source edits, registry changes и LLM-вызовов. Python primary-language gate остается обязательным; non-Python-primary workspaces попадают в `out_of_scope` и не входят в scored readiness, но отображаются в coverage. Summary публикует `avg_quality_score`, `worst_case_score`, `ready_threshold=0.92` и `ready_by_worst_case`; успешный статус возможен только если худший scored project проходит threshold и нет `needs_review`. Этот gate не запускает execution, Foundry, promote или source apply; он доказывает только pre-execution контрактную готовность цепочки.
...
59.3. L4.0 Cognitive Control Plane v0.1 представлен `runtime/cognitive_control_plane.py`, `runtime/l4_decision_table.py`, `runtime/semantic_evidence_pack.py` и L4 validation gate `runtime/l4_semantic_validation.py`. Он не является новой профессиональной ролью и не вызывает LLM. Вход для role pipeline: goal, role artifacts, ReviewFindings, optional PromptAdequacyGate и флаг llm_invoked. Выход: `CognitiveControlPlaneDecision` с `artifact_promotion_gate`, deterministic `role_transition`, `semantic_escalation` и `crystallization_backlog`. Role Pipeline обязан использовать `role_transition.next_action` как основной next action и сохранять decision в общем отчете. Для Stage 2 есть отдельный режим `prompt_to_product`: вход `PromptAdequacyGate + supported_template`, выход `prompt_product_gate`, deterministic `role_transition` (`build_verified_system_package`, `ask_clarification` или `stop_unsupported`), `semantic_escalation` и crystallization backlog. `runtime/verified_system_package.py` обязан строить package только по решению L4.0, а не по локальной проверке prompt внутри раннера. L4.0 эскалирует в L4.5 только при semantic uncertainty: unclear/unsupported prompt, no safe candidate, failed advisory backend, semantic rework after contracts passed, promotion block без contract failure, bounded prompt без поддержанного template или concrete bounded implementation prompt с `prompt_intake_uncertainty`. Перед вызовом L4.5 L4.0 формирует `SemanticEvidencePack`: prompt facts, failed gates, known templates, forbidden actions и authority=false. Любой `SemanticHypothesisProposal` возвращается в L4.0 как `L4SemanticValidationResult`, где проверяются contract validity, evidence, risks, confidence, return path, forbidden actions, prompt-boundary policy и отсутствие runtime mutation claims; validation содержит `policy_review` и human-readable explanation. Повторяемые approved transitions попадают в crystallization backlog и отражаются в `L4DecisionTable` как candidates для policy/template/gate/test.
59.4. L4.5 Semantic Reasoner v0.1 представлен контрактами `SemanticHypothesisRequest` и `SemanticHypothesisProposal` в `runtime/semantic_reasoner.py`, а артефакты resolution/developer handoff вынесены в `runtime/semantic_resolution_artifacts.py`; отдельный CLI `tools/semantic_reasoner_run.py` прогоняет request JSON через bounded runner, L4 validation и optional replay. Это не executor и не новая автономная роль с правами на действие. L4.5 вызывается только когда `CognitiveControlPlaneDecision.semantic_escalation.l4_5_required=true`; входом является source decision, trigger reasons, evidence context, allowed hypothesis types, forbidden actions, output contract и return path. Допустимые outputs: `SemanticHypothesisProposal` с successful existing resolution, developer improvement request, template mapping, clarification question, unsupported reason, legacy new template candidate, risk interpretation, architecture option, rework target или knowledge gap. Запрещенные действия: execute pipeline, edit user source tree, mutate registry, build package, promote capability, bypass prompt_product_gate или artifact_promotion_gate. Любая L4.5-гипотеза обязана вернуться в L4.0 через `L4SemanticValidationResult`; сама по себе она не запускает Stage 2/Stage 3 и не меняет runtime state. `tools/semantic_reasoner_run.py --use-model` и `tools/verified_system_package.py --use-l45-llm` включают реальный OpenAI-compatible L4.5 вызов через отдельный профиль `COGNITIVE_OS_L45_BASE_URL`/`COGNITIVE_OS_L45_MODEL`; default профиль L4.5 использует `deepseek/deepseek-chat` через `http://127.0.0.1:8000/v1`, не наследует L4 произвольно и не обращается к локальным L3.5 моделям. При provider failure L4.5 записывает ошибку в hardening и возвращается к deterministic proposal; raw model output нормализуется и валидируется тем же contract. Если model-backed proposal пропускает existing route, найденный deterministic resolver по тому же evidence, hardening выполняет `deterministic_existing_route_rescue` и сохраняет исходный `model_hypothesis_type` для replay/оценки качества. Model quality modes фиксируются явно: `deterministic`, `model_propose_only`, `model_with_human_review`, `blocked_model_untrusted`. Если proposal имеет тип `successful_existing_resolution`, L4.0 может записать `SuccessfulResolutionCandidate`; Stage 2 затем может через `FallbackAutonomyLoop` проверить уже существующий mapped route в sandbox и только после project-scoped tests вернуть release-ready package. KB admission требует повторных verified successes и review. Если proposal имеет тип `developer_improvement_request`, L4.0 может записать `DeveloperImprovementRequest` как задачу разработчику, но не меняет runtime сам. Если proposal имеет legacy тип `new_template_candidate`, Stage 2 может записать `Stage2TemplateBacklogItem` только после явного L4 validation route; этот путь оставлен для совместимости и не является первым действием при отказе `no_supported_package_template`. `unsupported_product_surface` возвращается в `ask_clarification`. Автоматическое добавление deterministic template, code generation или registry/template mutation запрещены. `runtime/semantic_replay.py` сохраняет `SemanticProposalReplay`, а `runtime/l45_semantic_benchmark.py` / `tools/l45_semantic_benchmark.py` измеряют deterministic semantic-loop cases и входят в portable acceptance.

59.4.4. Greenfield planning semantic fallback v0.1. Контур `UserPrompt -> ProductArchitectureRecord -> ProductTechnicalSpec` не имеет права считать `generic_product` успешным результатом. Если `config/greenfield_architecture_patterns.json` не дал конкретный pattern, `runtime/greenfield_role_pipeline.py` останавливает handoff в SpecWriter, помечает `semantic_gap.reason_code=greenfield_pattern_missing`, формирует `SemanticEvidencePack` и `SemanticHypothesisRequest`, прогоняет proposal через L4.0 validation и возвращает `DeveloperImprovementRequest`, если существующими средствами задачу нельзя надежно закрыть. `tools/greenfield_role_run.py --use-l45-llm` и `tools/verified_system_package.py --planning-only --use-l45-llm` разрешают model-backed гипотезу, но не дают модели права писать код, менять KB/config/registry или создавать готовое ТЗ поверх generic architecture.
59.4.5. Semantic Target Profiles v0.1: семантическая оценка first-slice target, ранжирование SpecWriter candidate и выбор domain contract family вынесены в `config/semantic_target_profiles.json`. Профиль задает `symbols`, `symbol_prefixes`, `symbol_contains_any`, `symbol_contains_all`, `path_contains_any`, optional `exclude_if_profile_ids`, score/ranking bonus/penalty, `benign_runtime_boundary`, причины, `contract_family`, typed `input_contract`/`output_contract`, `side_effect_policy`, `validation_gates` и `failure_modes`. `runtime/semantic_target_profiles.py` является generic loader/matcher; `runtime/target_quality.py`, `runtime/role_spec_writer_ranking.py` и `runtime/technical_spec_builder.py` потребляют его как API и не должны накапливать новые role/domain-specific branches для каждого нового семейства. `tools/config_doctor.py` проверяет загрузку и целостность профилей. Если новый случай нельзя выразить текущей схемой, правильная доработка - расширить универсальный профильный язык и тесты, а не зашивать конкретный проект или библиотеку в код.
59.4.1. L4.5 Live Quality Trial представлен `runtime/l45_semantic_comparison.py` и `tools/l45_semantic_compare.py`. Corpus semantic benchmark содержит prompt-boundary и unknown-template cases; deterministic report и model-backed report сравниваются как `L45SemanticComparisonReport` по same action, model/deterministic better, contract pass rate, fallback count, forbidden action strip count и interpretation. Текущий measured result не дает модели исполнительного доверия: deterministic route проходит 22/22, live model-backed `model_propose_only` режим вызывает модель только на escalated cases, после hardening совпадает с deterministic action на большинстве cases, но не показывает model_better. Следствие: L4.5 остается proposal source; repeated useful patterns переносятся в L4.0 rules/templates/tests только после replay review.
59.4.2. L4.5 Generated Corpus v0.2 представлен `runtime/l45_semantic_corpus.py` и CLI flags `tools/l45_semantic_benchmark.py --generated-corpus-size <N> --seed <seed> --corpus-profile <profile>`. Curated cases остаются portable smoke/acceptance set; generated corpus строится из seeded матрицы known templates, bounded unknown CLI/FastAPI prompts, clarification/unsupported boundaries и рискованных prompt-boundaries. Поддержанные профили: `balanced`, `risk_heavy`, `unknown_template_heavy`, `known_template_regression`. Generated 200-case corpus используется как local/nightly field trial, а не обязательный CI gate. Поверх benchmark output строятся два производных API-артефакта: `L45SemanticCorpusAnalyticsReport` агрегирует boundary/action/category counts, model usage и backlog signals; `L45RiskPolicyGapReport` фиксирует случаи, где risky/unsupported prompt попал бы в обычный template backlog без дополнительного gate. Нормальный результат gap report для текущей политики: `status=ok`, `gap_count=0`.
59.4.3. L4.5 Evaluation Suite v0.1 представлен `runtime/l45_semantic_eval_suite.py` и `tools/l45_semantic_eval_suite.py`. Он запускает несколько generated corpus profiles одним reproducible run, строит deterministic benchmark, analytics, risk-policy gap report и, при явном `--include-model`, model-backed benchmark plus `L45SemanticComparisonReport`. Выходной `L45SemanticEvaluationSuiteReport` содержит config, per-profile summaries, суммарные model/deterministic deltas, fallback count, общий risk-policy gap count и verdict (`deterministic_profiles_pass`, `model_underperformed_deterministic_route`, `model_adds_signal`, `model_provider_degraded`, `risk_policy_regression`). Модельный режим остается opt-in; даже при `model_propose_only` model output сравнивается и валидируется, но не становится authority для template admission, package build или runtime mutation. Benchmark case actual payload сохраняет `validation_failed_codes`, чтобы model failures были диагностируемы без ручного чтения replay.
...
59.6. Role Gate Modes v0.1 представлены `runtime/role_gate_runner.py`. `RoleGateReport` имеет режимы `advisory`, `strict` и `release_required`. `advisory` фиксирует failures как warning и не блокирует исследовательский прогон; `strict` блокирует acceptance role-chain при failed gate; `release_required` является выпускным режимом и требует чистого набора role artifacts перед promotion/release. Режим gate является свойством runtime-интерпретатора, а не hardcoded знанием конкретной роли.

59.7. Bounded LLM Sandbox Implementation v0.3 представлен `runtime/llm_sandbox_implementation.py`, `runtime/sandbox_operation_graph.py`, `runtime/sandbox_programmer_admission.py`, `tools/llm_sandbox_implementation.py` и registry `registry/sandbox_programmer_operations.json`. Если bounded adequate prompt не покрыт deterministic template/KB route, система может построить `SandboxImplementationPlan` и проверить маленький allowlisted implementation package в `artifacts/llm_sandbox_implementations/*`. Набор операций программиста вынесен в registry: code является интерпретатором operation records, а не местом ручного добавления каждого нового route. Каждый plan содержит `SandboxOperationGraph`: typed цепочку `read_input -> parse_input -> transform -> serialize_output -> write_output -> verify`, parser/serializer selection, input/output artifacts, side-effect boundaries, evidence и invariants. Этот graph является API между L4/L4.5, programmer, tester и admission gates; он не является prose report и не расширяет authority операции. `text_expression` проходит AST hardening и разрешает только безопасные конструкции (`text`, `str`, `len`, allowlisted text methods, простые slices/addition); profile-based операции не принимают expression и реализуются только allowlisted stdlib code paths. Текущие profiles: `line_sort`, `line_unique`, `csv_row_count`, `csv_sort_first_column`, `csv_select_first_two_columns`, `csv_filter_first_column_nonempty`, `csv_sum_second_column`, `csv_to_json_records`, `html_table_to_csv`, `json_extract_first_key`, `json_keys`, `json_pretty`. Registry не является произвольным Python execution channel. LLM может быть только hypothesis source; executable code не исполняется из raw model output, а создается из allowlisted contract. Результат имеет `promotion_allowed=false`, `source_code_changes=false`, `registry_changes=false`. Если verification проходит, `SandboxProgrammerAdmission` проверяет README/source/tests/fixtures, CLI boundary, negative test, project-scoped verification и invariants. Успешный admission может дать `release_ready_with_risks`, но не дает права менять user source, registry или KB. При `write=true` успешный admission дополнительно создает staged `KnowledgeCandidate` record type `successful_resolution_candidate` со статусом `collect_more_cases`; KB/template crystallization по-прежнему требует нескольких подтвержденных кейсов и review.

59.7.1. L4.5 Operation Normalizer for Sandbox Programmer v0.1 является узким fallback перед отказом `LLMSandboxImplementationResult`. Если deterministic registry match не найден и caller явно передал `use_model=true`, runtime вызывает L4.5 через `LocalInferenceConfig.from_l45_env()`; default model id - `deepseek/deepseek-chat`. Модель получает prompt и compact список records из `registry/sandbox_programmer_operations.json`, а вернуть может только JSON `{operation_id, confidence, reason}`. Код принимает proposal только если `operation_id` существует в registry и `confidence >= 0.55`; иначе результат остается controlled blocked. L4.5 не генерирует Python, не меняет registry/KB/source, не получает права на dependency install/network и не обходит `SandboxOperationGraph`, compile/pytest или `SandboxProgrammerAdmission`.

59.7.2. Sandbox Programmer Prompt Field Trial v0.1 представлен `runtime/sandbox_prompt_field_trial.py` и `tools/sandbox_prompt_field_trial.py`. Он прогоняет natural-language variants через deterministic registry matching и optional L4.5 normalizer, затем возвращает `SandboxPromptFieldTrialReport`: case statuses, selected operation/profile, strategy, model invocation, confidence, verification/tests status и агрегаты по verified/blocked/operation counts. Этот field trial не является KB admission и не auto-promote операций; он нужен для измерения качества prompt normalization и обнаружения слишком узких prefilters/match records.

59.7.3. Sandbox Operation Composition v0.1 поддерживает только allowlisted deterministic recipes, а не произвольный DAG. Recipes вынесены в `registry/sandbox_programmer_compositions.json`; runtime валидирует `match_all`, `match_any_groups`, 2-3 steps и то, что каждый step ссылается на существующую operation record из `registry/sandbox_programmer_operations.json`. Текущие recipes: `csv_filter_first_column_nonempty -> csv_select_first_two_columns -> csv_to_json_records` и `trim -> upper`. Результат остается обычным isolated sandbox package с compile/pytest, но `SandboxOperationGraph` обязан раскрывать несколько transform nodes (`transform_1`, `transform_2`, ...), чтобы tester/admission видели всю цепочку. L4.5 не получает права строить новые compositions; расширение списка recipes является registry-reviewed change с тестами.

59.7.4. Generated Product Quality Gate v0.1 представлен `runtime/generated_product_quality.py` и является product-facing выпускным контрактом для Stage 2. Пользовательский результат `Prompt -> Verified System Package` - это рабочий каталог generated project, а не промежуточные `GoalSpec`/`TechnicalSpec`/`ImplementationPlan`/review artifacts. Эти артефакты остаются API между уровнями и ролями. `GeneratedProductQuality` оценивает именно пакет: наличие source/tests/README, CLI/service entrypoint, явный input/output contract, controlled error path, run/test instructions, project-scoped verification, positive release decision, отсутствие runtime cache мусора в release directory и case-specific behavior. Порог `ready_threshold=0.92`; пакет ниже порога считается `needs_rework`, даже если отдельные role gates прошли. Для web research package gate дополнительно проверяет top-15 limit, explicit live search endpoint adapter, fixture-only offline tests, Markdown/JSON output, сохранение source links, handling empty/malformed inputs и документированную network policy.

59.8. Evaluation Corpus Expansion v0.2 представлен `runtime/evaluation_corpus.py`, `tools/evaluation_corpus.py`, `tools/evaluation_run_cognitive_os.py` и `tools/evaluation_run_direct_agent.py`. Команда `python tools/evaluation_corpus.py --root . --count 20 --write` создает недостающие задачи в `evaluation/` с одинаковым контрактом `prompt.md`, `direct_agent/`, `cognitive_os/`, `metrics.json`, `verdict.md`. `evaluation_run_cognitive_os.py` запускает Stage 2 Cognitive OS route и обновляет только `routes.cognitive_os`; `evaluation_run_direct_agent.py` запускает маленький прямой baseline без L4/KB/role gates и обновляет только `routes.direct_agent`. Direct baseline имеет право вернуть controlled `blocked`, если безопасный dependency/backend route не определен. Пустой `not_run` task допустим как placeholder, но завершенный evaluation обязан сравнивать одинаковый prompt и ограничения для direct route и Cognitive OS route.

59.9. Knowledge Usage Telemetry v0.1 представлена `runtime/knowledge_usage_telemetry.py` и `tools/knowledge_usage_report.py`. Runtime пишет append-only `KnowledgeUsageEvent` в `artifacts/telemetry/knowledge_usage.jsonl` для реальных KB touchpoints: skipped/gap preflight, architecture rule match и external knowledge probe. Событие содержит role, source, status, confidence, `rule_id`/`gap_id`, query и compact details. Summary report группирует usage по event type, role, source, status и top matched rules. Это измерительный слой для вопроса "как часто KB реально используется", а не admission gate и не источник новых прав.

59.10. Product Expectation Handoff v0.1 усиливает первые три роли для prompt-to-product задач. Architect обязан передавать в `ProductArchitectureRecord` не только компоненты и варианты архитектуры, но и `product_output_contract` с пользовательской формой результата, а также `real_world_edge_cases` с краевыми сценариями, которые вероятно сломают продукт. SpecWriter обязан сохранить эти поля в `ProductTechnicalSpec` и развернуть их в `acceptance_criteria` и `verification_strategy`. Это фиксирует правило: пользовательский результат проверяется до реализации, а `ProductArchitectureRecord` и `ProductTechnicalSpec` являются API между ролями. Для web research сценария текущий quality gate требует plain user query/default search adapter, one final summary, compact source list, Cyrillic/IRI URL handling, noisy aggregator pages и empty/malformed article negative tests.

59.11. Unified Cognitive OS Entry Route v0.1 является единой продуктовой точкой входа для пользовательского prompt. Она представлена `tools/cognitive_os.py`, `runtime/cognitive_os_entry.py` и конфигурацией `config/cognitive_os_entry_routes.json`. До запуска любого pipeline строится `CognitiveOSEntryRouteDecision`: входами являются raw prompt, optional `project_dir`, explicit mode, `PromptAdequacyGate` и optional `Stage2TemplateRoute`. Решение выбирается интерпретацией config routes, а не ручным выбором разработчика. Текущие pipelines: `prompt_to_product` (`PromptAdequacyGate -> Stage2TemplateRoute -> VerifiedSystemPackage`), `greenfield_architect_spec` (`UserPrompt -> ProductArchitectureRecord -> ProductTechnicalSpec`), `project_foundation_analysis` (`ProjectMapReport -> ArchitectureDecisionRecord -> TechnicalSpec`) и `clarification` (`PromptAdequacyGate -> ClarificationPacket`). Старые CLI (`verified_system_package.py`, `greenfield_role_run.py`, `role_foundation_run.py`, `goal_run.py` и probes/benchmarks) остаются низкоуровневыми runners/diagnostics, но не должны быть основным способом выбора стартового модуля для обычного пользовательского запроса. `CognitiveOSEntryRunReport` обязан сохранять route decision, pipeline result и invariants: `single_entrypoint_used=true`, `route_selected_by_config=true`, `direct_user_source_modification=false`.

### 13. Что намеренно не входит в первый MVP
Актуальный product-facing target: `Prompt -> Verified Local Automation Package`. Система должна принимать bounded prompt и возвращать локально проверяемый automation package или small service в sandbox: source, README, tests, verification report, product-quality gate, review/release decision и controlled refusal за пределами области.

В область этого MVP входят Python CLI tools, local small services без GUI, file/document/image/text/archive/table/structured-data automation, OCR/vision workflows с fixture/injectable tests, web/API clients и scraping только с fixture/mock acceptance, а также project analysis/improvement planning.

Первый product MVP не включает:
...
* GUI applications.
* SQL DB как обязательное runtime state.
* production deploy.
* mandatory live-network acceptance.
