...
python tools/goal_run.py --root . --goal "Analyze project map" --execute --interpret-project-llm --input-json "{\"path\":\"F:\\\\ubuntu\\\\test\\\\map\"}" --l4-model gpt-4o-mini
...
Runner выполняет layer-oriented acceptance scenarios: plugin contracts, registry doctor, happy path, quarantine/fallback, controlled stop без fallback, durable queue/worker pool, Foundry spec/candidate dry-run, dialogue memory, memory templates, deterministic template instantiation, Level 4 goal execution/deliberation, field-trial report, Project Analyzer benchmark, Knowledge Gap probe, GitHub L4 interpretation quality probe и known deterministic L4 routes (`list_files`, project-tree scan/analysis, markdown-to-text file, markdown-to-RTF file, fetch-links, translate-text, parse-pdf, spreadsheet-to-CSV, CSV-to-spreadsheet). Он пишет JSON/Markdown reports в `artifacts/acceptance/`.
...
52.0.7. Stage 3 Product Debug Loop исполняется через `runtime/product_debug_loop.py` и проверяется `tools/product_debug_loop_probe.py`. Loop принимает `VerifiedSystemPackage`, reference contract и выполняет только bounded package-local rework: `ProductFailureAnalysis -> ProductReworkPlan -> package repair -> project-scoped verification -> tester review -> refreshed package`. Поддержанные repair actions v0.1: `rewrite_readme_from_verified_package`, `add_missing_scenario_test_inside_generated_package`, `repair_api_contract_drift`, `rerun_project_scoped_verification`. Loop запрещает `edit_user_source_tree`, `edit_registry` и `promote_candidate`; инварианты результата: `sandbox_only=true`, `source_tree_changes=false`, `registry_changes=false`, `bounded_rework=true`. Acceptance gate `product_debug_loop_probe` намеренно повреждает FastAPI API contract (`/items/{key}` -> `/broken/{key}`), требует `api_contract_drift` blocker, применяет route repair и подтверждает `final_status=ok`.

52.0.8. Stage 3 Product Slice Benchmark исполняется через `tools/product_slice_benchmark.py` и проверяет текущий ограниченный corpus из трех product prompts: FastAPI key/value CRUD, FastAPI CSV aggregator и CLI text stats. Для каждого case требуется `status=ok`, `release_decision=slice_ready`, `requirements=satisfied`, `task_graph=complete`, `documentation_review=ok`, `scenario_verification=covered` и `product_debug_loop=not_needed`. Acceptance gate `product_slice_benchmark` требует `case_count=3`, `ok=3`, `failed=0`.
