...
52.0.5.1a. Profile-gated news-site scraper CLI v0.2 представлен route `news_site_scraper_cli`, teacher reference в `curricula/programmer_prompt_stage2/news_site_scraper_cli`, profile catalog `config/web_extraction_profiles.json`, routing interpreter `runtime/web_extraction_profiles.py` и template `runtime/greenfield_news_scraper_template.py`. Он заменяет подход "один scraper template на один сайт" для задач вида `https://3dnews.ru/ -> CSV`, но не считает любой URL новостным сайтом. Prompt классифицируется как `cli`, а package route допускается только если host имеет verified `target_kind=news` profile с evidence и link markers. URL без профиля, например новый или неоднозначный сайт, должен получить `prompt_product_gate.supported_template=null`, `semantic_escalation.l4_5_required=true` и `DeveloperImprovementRequest` на добавление web extraction profile вместо генерации фиктивного scraper package. Для известных профилей parser работает fixture-first по `article` и fallback news links, CSV contract фиксирован (`title,url,date,summary,source`), live network остается optional smoke через fetcher adapter с timeout/user-agent. Default verification не требует сети и проходит project-scoped compile/pytest; release decision может быть `release_ready_with_risks` из-за сетевого live-smoke риска.
...
Актуальный product-facing MVP target: `Prompt -> Verified Local Automation Package`. Система должна принимать bounded prompt и возвращать локально проверяемый automation package или small service в sandbox: source, README, tests, verification report, review/release decision и controlled refusal за пределами области.
