...
KB пополняется через admission gate (`runtime/knowledge_admission.py`). Новый record сначала становится staged `KnowledgeCandidate`; active KB merge требует нескольких подтвержденных кейсов, подтверждения внешнего учителя/корректировщика и Codex/developer approval. Автоматическое само-продвижение candidate в KB запрещено.
