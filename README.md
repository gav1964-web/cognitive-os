...
Role-by-project-type development now has an explicit sequencing lane. In the first narrow Python-owned lane, `cli_local_tool` plus `library_pure_transform`, Project Analyzer, Architect, and SpecWriter have reached the `9.7` promotion target with five blind projects and two independent reports per cell. Implementer, Tester, and Reviewer have measured `10.0` only on bounded derived transformation contracts; their `score_scope` is `bounded_transformation_contracts`, confidence is medium, and maturity remains usable until at least three project-native transformations are demonstrated. Web/API, provider SDK, framework/plugin, data/scientific/ML, async, stateful, external-I/O, and LLM/multi-agent strata remain required deferred work.
...
`SelfDevelopmentExperimentQueue` now joins prospective detection to that certification boundary before an experiment can be planned. It verifies the certification and its underlying holdout receipt, rejects tampered certificate payloads, digest-binds the detector result, maps allowlisted error families to target metrics and defers candidates outside the certified lane. The first prospective queue evidence receipt is `sha256:614904cb8cf361533519162f348da9c7bf57d0c828960432ec30e8384a42bc15`: status is `waiting_for_candidate`, all `5/5` authority checks pass and zero candidates are ready. This is a truthful negative result, not evidence of self-improvement; the next action is to collect a repeated independent error on `cli_local_tool` or `library_pure_transform`.
