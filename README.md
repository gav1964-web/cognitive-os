...
Role-by-project-type development uses an explicit sequencing lane. The 2026-09-04 narrow certificate recorded 12 matrix cells at `9.7+`, but its v1 evidence proved derived transformation and chain mechanics without preserving semantically auditable Project Analyzer, Architect, and SpecWriter work. The certificate is therefore revoked pending a v2 ground-truth holdout. Implementer, Tester, and Reviewer evidence remains useful regression evidence, not proof that the first three roles understand a project. Web/API, provider SDK, framework/plugin, data/scientific/ML, async, stateful, external-I/O, and LLM/multi-agent strata remain required deferred work.
...
Historical evidence snapshot (2026-08-30): `framework_plugin_build` completed three independent project-native sandbox repairs and passed the promotion rules in force at that time (`174` regression tests, Config Doctor `44/44`, zero source-project changes). Its matrix scores of `9.7/9.7/9.7/9.8/10.0/9.8` are retained for audit but are no longer certification authority: the first three values were derived from artifact presence and target continuity instead of semantic quality. Target-scoped admission still demonstrates bounded safety behavior, while automatic source apply and broad effect-heavy autonomy remain disabled.
...
Narrow maturity is separate from certification. `NarrowTypeCertification` v2 covers only `cli_local_tool` and `library_pure_transform`; every required role cell must be at least `9.7`, promotion eligible, backed by a disjoint multi-lineage holdout receipt, free of generated function stubs and role regression, and supported by auditable semantic role artifacts plus an evaluated project change. The former v1 certificate does not satisfy those conditions and is revoked. Broad strata remain explicitly deferred. `SelfDevelopmentExperiment` compares baseline and candidate metrics for L0/L1 and rejects any regression or generated function stub before L0 reviewer staging can begin.

`SelfDevelopmentExperimentQueue` joins prospective detection to that certification boundary before an experiment can be planned. It verifies the certification and its underlying holdout receipt, rejects obsolete or tampered certificate payloads, digest-binds the detector result, maps allowlisted error families to target metrics and defers candidates outside the certified lane. With the v1 narrow certificate revoked, receipt `sha256:c22c30b2d32bc41efdbf2527cf19b164516c0fe16d526cf5134fa057857f1144` records a blocked queue with zero ready candidates; it distinguishes the certificate's historical declared status from its rejected authority status. The framework candidate remains deferred until a v2 semantic certificate exists.
...
That negative audit is now superseded by receipt `sha256:c08272032b9734de5040085c2027413eb0be50bd372bbdea4a4e5072da190014`: all five transition producers build or verify digest-bound interpreter traces, coverage is `5/5`, and bypass gaps are zero. Configured role execution chains traces across artifacts; project-development handoff, control-plane decisions and recovery returns fail closed to `controlled_stop` when prior authority is invalid. The task-tree builder is explicitly modeled as an implementation substage and may route directly to plan review without granting execution authority. This is static and regression-tested boundary authority, not proof that every future transition is semantically optimal.

`framework_plugin_build` remains a separate lane. Its repeated recognition-gap candidate spans four independent projects, but the old matrix scores are not semantic role evidence. The v2 diagnostic holdout scores Project Analyzer `4.0`, Architect `2.0`, and SpecWriter `1.67`; it preserves full role artifacts and shows that no project-development change was evaluated. Framework certification is revoked until a ground-truth holdout demonstrates correct project understanding, a relevant architecture decision, an executable specification, and a real bounded sandbox change.

The 2026-09-06 prospective challenge reused the local corpus first. Only two untouched local CLI candidates remained, so measured shortage authorized three revision-pinned external CLI acquisitions; the frozen manifest receipt is `sha256:6f3834dad93e64ff05e983cf1be6769f033e5211d3ab7203503a81b7a39e5be8`. Campaign receipt `sha256:f52ed3bcb4b7fabc6e4b90656f0e72826a53e477d5942e3c2ea5805999fbdf9b` executed six acquisition cases and matched the expected type in `3/6`; the mismatches remain evidence, not discarded failures. Declared distribution scripts now override only low-confidence framework classification, preserving source-backed packaging backends.

Prospective detection now evaluates only the latest post-cutoff report per project. Receipt `sha256:fea5bc684bb0393cd7451ab483c9a65bbd682388626520a71b532a60004a429f` supersedes 24 stale reports, retains one framework recognition-gap candidate over four independent projects and leaves the narrow pure-transform signature at `1/3`. Queue receipt `sha256:881aa221306e21bca1c0ced74d38e8e25a761d1ce36f1cae035b0460db547e2f` therefore has `ready_count=0`; no L0/L1 experiment or KB mutation was run.

The earlier framework readiness checkpoint remained `holdout_required` under receipt `sha256:d18873738e2cb6b8a3a299aa5d4dc63bacbcaca277676acb3a32a8bb5de51766`. The dedicated `FrameworkPluginCertification` gate requires all six cells to be mature at target with complete evidence plus a verified ledger receipt for a multi-lineage independent holdout, role-chain continuity, zero generated function stubs, bound inputs and no role regression. Receipt `sha256:7f9b2efb4825a039efa3fba1438a5bdacfcc11d4916c6442ed2867893e86937c` records that historical readiness-only state; scores alone did not grant certification.

The 2026-09-06 framework/plugin follow-up originally reported a `3/3` holdout and issued certificate receipt `sha256:2b753d022452ef95039e308a6048e3c89aea7eb256a71a4d883e90106f5d0ace`. A subsequent semantic audit found that the first three role scores were constants granted from recognition and artifact continuity, while the holdout selected incidental callable targets and performed verification-only execution. The certificate is therefore revoked, not silently replaced. The v2 holdout retains all 49 unrelated mature cells and zero generated stubs, but correctly returns `evidence_required`: analyzer, architect and spec-writer worst-case diagnostic scores are `4.0`, `2.0` and `1.67`; all three full role artifacts are now retained with content digests. Reassessment receipts are `sha256:7b3515d984be0e4ff85a44d7dfe03fe4bcb593f52d9b9ad9948adbaf4d6acb05` and `sha256:44d08a0cdd3c2e4fafb65ac7c99bae39e80d688f66acb0262eb1326b64495bca`. Framework/plugin maturity now requires a project-specific semantic holdout and a real sandbox change; general role-matrix observations remain evidence but are not certification authority.
...
Narrow-lane reassessment (2026-09-06): the v1 holdout and certificate receipts still verify cryptographically, and their zero-stub, lineage, interaction, and downstream mutation observations remain valid. They do not preserve semantic evidence or complete role artifacts for Project Analyzer, Architect, and SpecWriter, and the derived probes did not evaluate a project-development change. `NarrowTypeCertification.status` is therefore `revoked`; reassessment receipt `sha256:51c039cec3a6f65acef3dd88dc9286abe2923c95b5a71458098b70dbddbac3d8` records the decision. Re-certification requires a v2 ground-truth holdout. Broad strata, automatic source apply and Web UI remain deferred.

The first semantic calibration covered four known project-native repairs: `platformdirs`, `marshmallow`, `autoflake`, and `isort`. Its initial v1 evaluator incorrectly awarded all six roles `10.0` by treating complete artifacts and target continuity as semantic success. Semantic evidence v2 separates structural completeness from validated competence and requires causal diagnosis, a defect-specific architecture repair design, an executable specification delta, and outcome evidence. Reassessment receipt `sha256:319a910da7d64b21f2840ba7ab59ee42d7b10d80cb59068a584b9bdd378f7aa6` now scores Project Analyzer `8.0`, Architect `6.0`, SpecWriter `6.0`, and the actually verified downstream roles `10.0`. The calibration status is `evidence_required`; its consumed cases may be used for training but not fresh certification evidence.

The first owner-independent defect holdout qualifies two real upstream failures: `tox` for `cli_local_tool` and `more-itertools` for `library_pure_transform`. `pipx` is excluded by a bounded pytest timeout and `pyTooling` by its unavailable Python 3.11 runtime; neither exclusion is counted as success. Intake binds pytest subtests, class test methods and unique local wildcard re-exports, and prevents fixture setup errors from borrowing repair authority from a test body. The initial honest v2 baseline was `8.0/6.0/6.0/4.0/0.0/2.0`: identity and target localization worked, but causal diagnosis and repair design did not. The consumed cases now feed two explicitly training-only causal patterns. They produce source-bound causal hypotheses, preserve defect-specific repair designs through the Architect handoff and replace the impossible empty-reducer requirement with a concrete mutation proposal. The proposals have no execution authority and remain outside `allowed_operator_ids`. Receipt `sha256:02781dc142896fd8c86b9a722e0fa550f7acccb9c5ea8626237411ceb129acff` therefore raises only the first three roles to `9.2/7.0/6.5`; downstream stays `4.0/0.0/2.0`. Next comes independent proposal validation and sandbox execution on consumed training defects, followed by a second fresh owner-independent defect per stratum. Certification, broad strata and Web UI remain deferred.

Consumed-case causal replay (2026-09-07) adds an explicit `--authorize-training-replay` boundary. Without that flag the two proposed operators remain blocked; with it they can mutate only a sandbox copy, cannot apply source, and cannot promote KB records. The first more-itertools attempt exposed an under-scoped design: the empty-input contract spans `split_before`, `split_after`, and `split_when`, so the diagnosis and mutation were corrected to a bounded same-module function family before the full native suite passed. Tox passes its original failing node and the complete affected `tests/config/loader/ini` subsystem; the repository-wide suite is not claimed because two Windows locale assertions and the bounded runtime prevent a portable full pass. Training receipt `sha256:fbb645657b1122d7a5715eca04b3a2d835688004d4d7e0e11c462b8556d5682e` therefore remains `evidence_required` and applies explicit training/scoped-regression caps: Project Analyzer `9.4`, Architect `8.5`, SpecWriter `8.5`, Implementer `8.8`, Tester `8.5`, Reviewer `8.5`. These are demonstrated known-case capabilities, not transfer maturity. The next gate is one fresh owner-independent defect per stratum with no preloaded fix pattern; only that evidence may remove the training caps.

The 2026-09-08 transfer audit adds a digest-bound `ProblemOutcomeContract` from Project Analyzer through Reviewer. It binds the failure signature, target, permitted scope, expected outcome, replay and regression obligations, and remains explicitly non-authorizing. Any digest drift, target drift, absent baseline evidence, missing replay, missing regression, or generated function stub fails closed. The top-level development report now retains all downstream role artifacts instead of discarding the ImplementationPlan, TestPlan, ProgrammerTaskTree, and ReviewFindings.

Three owner-independent defects were selected from the existing local corpus before evaluation: Rich, Click, and pytest-httpserver. All three retained one causal contract and target through the complete planning chain, but none had an unseen executable repair validated in this audit. Rich, Click, and pytest-httpserver therefore ended `research_required`; no source was changed and no KB record was promoted. A tempting Deepmerge result was explicitly rejected as transfer evidence because its generic failure kind and operator were already present in KB even though the project name was absent. Semantic receipt `sha256:d60886ef0567bbfbb005424e92c60b0df2433aa5d89385a4df95b739e1f61fad` reports Project Analyzer `8.0`, Architect `5.0`, SpecWriter `6.0`, Implementer `4.0`, Tester `0.0`, and Reviewer `2.0`. These are the current unseen-defect scores. The next gate requires at least two independent unseen cases per stratum and a bounded hypothesis/design path, potentially LLM-assisted, whose raw output has no execution authority and must pass deterministic evidence, sandbox, regression, and external-review gates.

The bounded L4.5 hypothesis path is now implemented behind the explicit `tools/project_development.py --use-l45-llm` flag. It runs only for a failure-backed KB miss, sends one source-bounded target and failure signature, and accepts a closed JSON schema containing a causal mechanism, abstract repair mechanism, mutation preconditions, preserved behavior, residual risks and confidence. Target or signature drift, unknown fields, patch/code/command fields, malformed contracts and confidence below `0.6` are rejected. An accepted response remains `hypothesis_only`, leaves `allowed_operator_ids` empty, cannot execute, cannot apply source and cannot promote memory. Direct project-development and profile-evaluation CLIs now invoke the managed gateway bootstrap before using a matching loopback profile; remote and unrelated loopback endpoints are not started. Live smoke `llm_profile_eval_20260908T070103388061Z.json` started the configured gateway when absent, then recorded `already_running` and received valid JSON from `deepseek/deepseek-chat`. This proves connectivity and lifecycle behavior, not unseen-task role quality, so no role-score increase is claimed.

Training replay re-audit (2026-09-08) closes the known-case causal gap for Click, Autoflake, isort, Platformdirs, and Marshmallow. `ProjectFailureEvidencePacket` binds the stable repeated signature, exact AST target, failing test source, traceback/assertion evidence, and source digests before either a KB reducer or an LLM hypothesis can enter an explicitly authorized sandbox replay. The causal matcher now consumes that packet, and qualified method references remain intact during TestPlan alignment. All five consumed projects produced one bounded sandbox patch, passed the original failing test and full native regression, passed the generated-function-stub gate, and preserved the source project digest. Semantic report `sha256:842b7b88716714054a8a6c0e932a9c6302c5c72ca05ba7e60c00bae25fcb02cb` records structural `10.0` for all 30 role-case cells, but the declared `training_replay` split correctly caps published role scores at Project Analyzer `9.4`, Architect `8.5`, SpecWriter `8.5`, Implementer `9.0`, Tester `8.8`, and Reviewer `8.8`. Status remains `evidence_required`; unseen scores remain `8.0/5.0/6.0/4.0/0.0/2.0` until a fresh owner-independent holdout removes those caps.

Fresh-evidence update (2026-09-08): two local-first campaigns executed eight revision-frozen projects. Three were clean and five were environment-blocked; none produced a qualified fresh defect, so role scores did not change. During follow-up acquisition, the historical Invoke integer-help defect exposed and corrected warning-frame target binding, re-export alias resolution, vendored-source health ranking, legacy `setup.py` identity, and legacy-regression status handling. The complete role chain then produced one sandbox-only `add_cli_int_help_placeholder` patch; its source blob exactly matches upstream fix `de0eadaa64813d841cffa50fb3cc9774c763ac8e`, and the targeted plus three-case compatibility regression passed. Manifest `config/historical_defect_acquisition_invoke_20260908.json` records this as consumed acquisition evidence, not holdout evidence. Known-case replay breadth is now six projects, while the authority-bearing unseen scores remain `8.0/5.0/6.0/4.0/0.0/2.0`; the next gate is two qualified fresh owner-independent defects per narrow stratum.
