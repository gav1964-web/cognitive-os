...
The first v2 semantic calibration now passes four known project-native repairs: `platformdirs`, `marshmallow`, `autoflake`, and `isort`. Full Project Analyzer, Architect, and SpecWriter artifacts are retained with content digests; the downstream reports bind the same project and target to prepared patches, targeted replay, complete native regression, unchanged source digests, and zero generated function stubs. Failure-repair TechnicalSpec generation now replaces stale generic requirements, acceptance traceability, and verification plans with a three-criterion target-bound repair contract. The worst known-case score is `10.0` for all six roles, recorded by calibration receipt `sha256:a6a9830866fc751599075f3b985349bc730c7236b1e2b1d3849f7b07f673cb82`. This is not certification: the CLI pair has two repository lineages but one owner (`PyCQA`), and the frozen unseen corpus does not yet contain verified repair defects. The certificate remains revoked until a fresh owner-independent defect holdout passes the independent semantic evaluator.

The first owner-independent v2 defect holdout qualifies two real upstream failures: `tox` for `cli_local_tool` and `more-itertools` for `library_pure_transform`. `pipx` is excluded by a bounded pytest timeout and `pyTooling` by its unavailable Python 3.11 runtime; neither exclusion is counted as success. Intake now binds pytest subtests, class test methods and unique local wildcard re-exports, and prevents fixture setup errors from borrowing repair authority from a test body. Project identity evidence now outranks incidental internal capability markers, archetype ground truth is scored explicitly, and failure-repair IO contracts prefer source usage over `Any`. Receipt `sha256:34336f4401ea23e1d5f80cf9c7f2bb151d2c78674c76c4991f51e9c0fcb5e51a` gives Project Analyzer, Architect and SpecWriter `10.0`, but Implementer `4.0`, Tester `0.0` and Reviewer `2.0`: both unknown reducer shapes stop safely before source mutation. Status remains `evidence_required`, because each stratum has only one qualified owner/lineage and no evaluated project change. The next milestone is a second qualified independent defect per stratum plus verified bounded reducers and complete downstream evaluation; certification, broad strata and Web UI remain deferred.
