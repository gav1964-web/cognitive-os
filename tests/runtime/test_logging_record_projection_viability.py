...
def test_logging_formatter_in_handlers_module_allows_bounded_record_mutation():
    context = {
        "dependency_readiness": {"status": "ready"},
        "snippet": {
            "target_binding": "method_symbol",
            "owner_class": "MongoFormatter",
            "text": (
    ...
    result = first_slice_viability("mongolog/handlers.py:format", context)

    assert result["status"] == "eligible"
    assert result["reselection_required"] is False
    assert any(row["rule_id"] == "logging_formatter_record_boundary" for row in result["matched_rules"])
