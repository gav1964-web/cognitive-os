...
from tools.github_tester_probe import _quality_score, _run_case


ROOT = Path(__file__).resolve().parents[2]


def test_tester_curriculum_local_three_scores_teacher_references():
    report = run_tester_curriculum(
        root=ROOT,
        curriculum_dir=ROOT / "curricula" / "tester_local_3",
        write=True,
    )

    assert report["status"] == "ok"
    assert report["milestone"] == "Tester Curriculum Local-3 v0.1"
...
def test_tester_curriculum_cli_writes_report():
    result = subprocess.run(
        [
            sys.executable,
            str(ROOT / "tools" / "tester_curriculum.py"),
            "--root",
            str(ROOT),
            "--write",
        ],
...
def test_tester_curriculum_external_three_when_projects_exist():
    curriculum_dir = ROOT / "curricula" / "tester_external_local_3"
    ...
    report = run_tester_curriculum(root=ROOT, curriculum_dir=curriculum_dir, write=False)

    assert report["status"] == "ok"
    assert report["milestone"] == "Tester Curriculum External-3 v0.1"
    assert report["project_count"] == 3
    assert report["summary"]["worst_case_score"] >= 0.92
    assert report["summary"]["ready_by_worst_case"] is True
    assert report["summary"]["backlog_items"] == 0
...
def test_tester_curriculum_rejects_ground_truth_reference(tmp_path):
    ...
    (case_dir / "teacher_reference.json").write_text(
        json.dumps(
            {
                "schema_version": "tester_curriculum.v1",
                "project_dir": project_dir.as_posix(),
                "teacher_profile": "tester",
    ...
    with pytest.raises(ValueError, match="reference_quality='teacher_reference_not_ground_truth'"):
        run_tester_curriculum(root=ROOT, curriculum_dir=tmp_path)


def test_github_tester_probe_marks_rust_workspace_out_of_scope(tmp_path):
...
def test_github_tester_quality_score_is_capped_and_requires_binding():
    test_plan = {
        "artifact_type": "TestPlan",
        "role": "tester",
        "contract_test_matrix": [{"id": "C1"}],
        "negative_tests": [{"id": "N1"}],
        "acceptance_tests": [{"id": "A1"}],
        "smoke_checklist": [{"id": "S1"}],
    }
