...
def test_executable_acceptance_allows_none_for_qualified_optional_alias(tmp_path: Path):
    project = tmp_path / "project"
    project.mkdir()
    (project / "module.py").write_text("def lookup():\n    return None\n", encoding="utf-8")
    plan = _plan("module.py:lookup", {}, malformed=False)
    plan["executable_acceptance"]["obligations"][0]["expect"] = {"result": "t.Optional"}
