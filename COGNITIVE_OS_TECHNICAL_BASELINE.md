...
Роль `implementer` в текущем MVP является `Implementer Planner` и преобразует `TechnicalSpec -> ImplementationPlan`: patch scope, expected files, implementation units, change plan, quality gates, verification commands, rollback plan и acceptance mapping. Она не пишет код сама; ее задача — подготовить bounded work package для отдельного исполнения.
