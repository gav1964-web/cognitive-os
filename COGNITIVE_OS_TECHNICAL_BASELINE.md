...
L4.5 обязан быть видимым контрактным переходом. Если `semantic_escalation.l4_5_required=true`, runtime формирует `SemanticEvidencePack` и `SemanticHypothesisRequest`: source decision, trigger reasons, evidence context, allowed hypothesis types, forbidden actions, output contract и return path. Evidence pack содержит prompt facts, failed gates, known templates и authority=false, чтобы L4.5 не фантазировал за пределами фактов и не получал исполнительных прав. L4.5 может вернуть только bounded `SemanticHypothesisProposal`; он не имеет права исполнять pipeline, строить package, менять source tree или registry, promote capability или обходить L4.0/L3.5/L2 gates. Возврат всегда идет обратно в L4.0 через `L4SemanticValidationResult`: L4.0 отдельно проверяет contract validity, evidence, risks, confidence, return-to-gate, forbidden actions, prompt boundary policy и отсутствие claims о runtime mutation; результат содержит machine checks, `policy_review` и human-readable explanation. Реальный model-backed L4.5 включается только явно и работает через OpenAI-compatible gateway; raw model output нормализуется, forbidden actions отбрасываются, proposal валидируется, а при ошибке provider выполняется deterministic fallback. Режимы качества модели фиксируются явно: `deterministic`, `model_propose_only`, `model_with_human_review`, `blocked_model_untrusted`. Для Stage 2 proposal типа `new_template_candidate` превращается в `Stage2TemplateBacklogItem` только если `L4SemanticValidationResult.status=accepted` и `accepted_action=record_template_backlog`; backlog не является разрешением на автоматическую генерацию template или изменение runtime. Если prompt boundary равен `bounded_with_risks`, L4.0 понижает обычный backlog-route до `record_template_backlog_requires_human_review`; если boundary равен `unsupported_product_surface`, L4.0 маршрутизирует результат в `ask_clarification`, а не в template backlog. `SemanticProposalReplay` сохраняет request/evidence/proposal/hardening/validation/outcome для сравнения моделей и промптов. `L45SemanticBenchmarkReport` измеряет deterministic cases верхнего semantic-loop и входит в portable acceptance. Перевод backlog candidate в поддержанный template проходит через `Stage2TemplateAdmissionResult`: teacher-reference quality, deterministic template availability, isolated scaffold, project-scoped tests, tester review и отсутствие source/registry mutations.
