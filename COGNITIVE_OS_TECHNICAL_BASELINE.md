...
L4.5 обязан быть видимым контрактным переходом. Если `semantic_escalation.l4_5_required=true`, runtime формирует `SemanticHypothesisRequest`: source decision, trigger reasons, evidence context, allowed hypothesis types, forbidden actions, output contract и return path. L4.5 может вернуть только bounded `SemanticHypothesisProposal`; он не имеет права исполнять pipeline, строить package, менять source tree или registry, promote capability или обходить L4.0/L3.5/L2 gates. Возврат всегда идет обратно в L4.0 через `L4SemanticValidationResult`: L4.0 отдельно проверяет contract validity, evidence, risks, confidence, return-to-gate, forbidden actions и отсутствие claims о runtime mutation. Реальный model-backed L4.5 включается только явно и работает через OpenAI-compatible gateway; raw model output нормализуется, forbidden actions отбрасываются, proposal валидируется, а при ошибке provider выполняется deterministic fallback. Для Stage 2 proposal типа `new_template_candidate` превращается в `Stage2TemplateBacklogItem` только если `L4SemanticValidationResult.status=accepted` и `accepted_action=record_template_backlog`; backlog не является разрешением на автоматическую генерацию template или изменение runtime. Перевод backlog candidate в поддержанный template проходит через `Stage2TemplateAdmissionResult`: teacher-reference quality, deterministic template availability, isolated scaffold, project-scoped tests, tester review и отсутствие source/registry mutations.
