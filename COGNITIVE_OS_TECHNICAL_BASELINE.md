...
Contract Registry реализуется в `runtime/contract_registry.py` как производный источник истины поверх plugin manifests, JSON Schemas и межслойных packet routes. Он не дублирует `registry/capabilities.json` вручную и не становится вторым mutable registry. Причина: один mutable реестр быстро приведет к drift, а производный contract catalog можно пересобрать и проверить в любой момент.
