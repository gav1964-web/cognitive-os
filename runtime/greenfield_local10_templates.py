...
def _test_core(case_name: str) -> str:
    return {
        ...
        "duplicate_file_finder": "from duplicate_finder.finder import duplicate_groups\n\ndef test_duplicate_groups_exclude_unique_files(tmp_path):\n    (tmp_path/'a.txt').write_text('x'); (tmp_path/'b.txt').write_text('x'); (tmp_path/'c.txt').write_text('y')\n    groups = duplicate_groups(str(tmp_path))\n    assert groups == [['a.txt', 'b.txt']]\n    assert all('c.txt' not in group for group in groups)\n",
        "batch_renamer_cli": "import pytest\nfrom batch_renamer.renamer import apply, plan\n\ndef test_plan_and_apply(tmp_path):\n    (tmp_path/'a.txt').write_text('x')\n    assert plan(str(tmp_path)) == [('a.txt', 'renamed_a.txt')]\n    apply(str(tmp_path)); assert (tmp_path/'renamed_a.txt').is_file()\n\ndef test_path_traversal_is_rejected(tmp_path):\n    (tmp_path/'a.txt').write_text('x')\n    with pytest.raises(ValueError):\n        apply(str(tmp_path), '../bad_')\n",
        "json_config_merger": "import pytest\nfrom json_config_merger.merger import merge, run_cli\n\ndef test_recursive_merge():\n    assert merge({'a': {'b': 1}}, {'a': {'c': 2}}) == {'a': {'b': 1, 'c': 2}}\n\ndef test_invalid_top_level_json_is_rejected(tmp_path):\n    base = tmp_path / 'base.json'; override = tmp_path / 'override.json'\n    base.write_text('[]'); override.write_text('{}')\n    with pytest.raises(ValueError):\n        run_cli(f'{base};{override}', str(tmp_path / 'out.json'))\n",
