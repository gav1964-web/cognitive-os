...
from cognitive_replay.environment import prepare_environment, validate_environment_profile
...
def test_environment_uses_only_frozen_wheels_and_ignores_host_plugins(tmp_path: Path, monkeypatch, profile_factory) -> None:
...
def test_modified_wheel_is_rejected_before_environment_creation(tmp_path: Path, profile_factory) -> None:
    profile = profile_factory(tmp_path)
...
def test_python_requirement_is_checked_before_install(tmp_path: Path, profile_factory) -> None:
    profile = profile_factory(tmp_path)
    project = tmp_path / "project"
    project.mkdir()
...
def test_unavailable_dependency_closure_is_not_ready(tmp_path: Path, profile_factory) -> None:
    profile = profile_factory(tmp_path)
...
def test_environment_version_drift_is_rejected(tmp_path: Path, profile_factory) -> None:
    profile = profile_factory(tmp_path)
